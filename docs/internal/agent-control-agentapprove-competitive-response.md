...
| 20 | Group MCP server and tool context | Enforce/Fleet | Implemented and published (AMC-1482): one fail-closed projection over exact signature-verified ToolHub bytes derives stable native/MCP tool and server identities, groups existing list surfaces, and adds MCP server/provider context to CGX without a registry, discovery service, protocol importer, or proof claim |
| 21 | Show compound-command blast radius | Enforce | Implemented in AMC-1483: one bounded shared parser evaluates every shell segment through existing signed ToolHub and policy controls, applies the most restrictive outcome, seals a privacy-safe review, and fails closed without partial steps on unsupported syntax or untrusted authority |
