...
Archives are written to `dist/installers/archives/` and install AMC from the included AMC npm tarball. This works even before the public AMC npm registry package is available, though npm still needs access to AMC's public runtime dependencies unless they are already cached or mirrored.
...
Use the Windows desktop installer archive generated by `npm run package:desktop`, or install WSL2 with Ubuntu and follow the Ubuntu instructions above.
