...
AMC provides 1,163 public CLI command paths in the live command inventory.
...
| 360 | `amc eval run` | One-shot evaluation: read amcconfig.yaml, run all diagnostic tests, output results |
| 361 | `amc eval status` | Show imported eval coverage per AMC dimension |
| 362 | `amc evidence` | Evidence lifecycle workflows |
| 363 | `amc evidence collect` | Guided wizard to connect your agent and capture evidence |
| 364 | `amc evidence decisions` | List and inspect decision receipts generated by full-score runs |
| 365 | `amc evidence decisions inspect` | Inspect one decision receipt by receipt id or run id |
| 366 | `amc evidence decisions list` | List persisted decision receipts |
| 367 | `amc evidence decisions observe` | Update open decision receipts with observed outcomes from a later full-score run |
| 368 | `amc evidence episodes` | List, inspect, and export lifecycle evidence episodes |
| 369 | `amc evidence episodes export` | Export one EpisodeRecord as JSON or Markdown |
| 370 | `amc evidence episodes inspect` | Inspect one EpisodeRecord by episode id, lifecycle id, or run id |
| 371 | `amc evidence episodes list` | List persisted EpisodeRecord evidence objects |
| 372 | `amc evidence export` | Export verifier-ready evidence (json|csv|pdf) |
| 373 | `amc evidence finding-proofs` | List, inspect, and export finding proof chains |
| 374 | `amc evidence finding-proofs export` | Export finding proofs as JSON |
| 375 | `amc evidence finding-proofs inspect` | Inspect one finding proof by proof id, finding id, run id, or question id |
| 376 | `amc evidence finding-proofs list` | List persisted finding proof chains |
| 377 | `amc evidence help` | Show high-signal evidence command groups |
| 378 | `amc evidence lifecycle` | List, inspect, and export full lifecycle run artifacts |
| 379 | `amc evidence lifecycle export` | Export one lifecycle artifact as JSON |
| 380 | `amc evidence lifecycle inspect` | Inspect one lifecycle artifact by lifecycle id or run id |
| 381 | `amc evidence lifecycle list` | List persisted lifecycle run artifacts |
| 382 | `amc evidence lifecycle-receipts` | List, inspect, and export lifecycle proposal, validation, commit, rollback, and monitor receipts |
| 383 | `amc evidence lifecycle-receipts export` | Export lifecycle receipts as JSON |
| 384 | `amc evidence lifecycle-receipts inspect` | Inspect one lifecycle receipt by receipt id, run id, or lifecycle id |
| 385 | `amc evidence lifecycle-receipts list` | List persisted lifecycle change receipts |
| 386 | `amc evidence observability` | List and inspect component, experience, and decision observability records |
| 387 | `amc evidence observability inspect` | Inspect one observability lane record by observability id, lifecycle id, or run id |
| 388 | `amc evidence observability list` | List persisted observability lane records |
| 389 | `amc evidence verify` | Run full workspace verification suite |
| 390 | `amc executive` | Executive and board-ready AMC artifacts |
| 391 | `amc executive brief` | Generate a board-ready one-page executive brief from a diagnostic run |
| 392 | `amc experiment` | Deterministic baseline vs candidate experiments |
| 393 | `amc experiment analyze` | Analyze latest experiment run |
| 394 | `amc experiment create` | Create an experiment |
| 395 | `amc experiment gate` | Evaluate latest experiment run against gate policy |
| 396 | `amc experiment gate-template` | Write an experiment gate policy template |
| 397 | `amc experiment list` | List experiments |
| 398 | `amc experiment optimize` | Create governed optimizer candidates from a Fixer RCA report |
| 399 | `amc experiment optimizer-list` | List governed optimizer runs |
| 400 | `amc experiment optimizer-show` | Show a governed optimizer run |
| 401 | `amc experiment run` | Run deterministic experiment against signed casebook |
| 402 | `amc experiment set-baseline` | Set experiment baseline config |
| 403 | `amc experiment set-candidate` | Set experiment candidate signed config overlay |
| 404 | `amc experiment-architecture` | Run a controlled architecture comparison experiment |
| 405 | `amc experiment-architecture-probes` | List the standard probe set for architecture experiments |
| 406 | `amc explain` | Plain-English explanation for a diagnostic question (example: AMC-2.1) |
| 407 | `amc export` | Export policy packs and badges |
| 408 | `amc export badge` | Export deterministic maturity badge SVG for a run |
| 409 | `amc export policy` | Export framework-agnostic North Star policy integration pack |
| 410 | `amc federate` | Offline federation sync operations |
| 411 | `amc federate export` | Export offline federation sync package (.amcfed) |
| 412 | `amc federate import` | Import and verify federation package |
| 413 | `amc federate init` | Initialize federation identity and signed config |
| 414 | `amc federate peer` | Federation peer trust anchors |
| 415 | `amc federate peer add` | Add a peer publisher public key |
| 416 | `amc federate peer list` | List federation peers |
| 417 | `amc federate verify` | Verify federation config signature |
| 418 | `amc federate verify-bundle` | Verify .amcfed package |
| 419 | `amc firewall` | Runtime protection for live agent traffic |
| 420 | `amc firewall check` | Evaluate a request or response payload against Runtime Firewall |
| 421 | `amc firewall disable` | Disable Runtime Firewall for this workspace |
| 422 | `amc firewall enable` | Enable Runtime Firewall in observe, warn, or block mode |
...
| 430 | `amc fleet contradictions` | Detect cross-agent contradictions |
| 431 | `amc fleet dag` | Visualize orchestration delegation graph |
| 432 | `amc fleet graph` | Typed multi-agent graph operations |
| 433 | `amc fleet graph list` | List saved typed multi-agent graphs |
| 434 | `amc fleet graph show` | Inspect the latest typed multi-agent graph |
| 435 | `amc fleet graph validate` | Validate the latest typed multi-agent graph |
| 436 | `amc fleet graph write` | Write the latest typed multi-agent graph from a JSON file |
| 437 | `amc fleet handoff` | Manage handoff packets |
| 438 | `amc fleet health` | Show fleet health dashboard aggregates |
| 439 | `amc fleet init` | Create and sign .amc/fleet.yaml |
| 440 | `amc fleet lifecycle` | Fleet parent/child lifecycle evidence |
| 441 | `amc fleet lifecycle list` | List parent fleet lifecycle artifacts |
| 442 | `amc fleet lifecycle show` | Inspect one parent fleet lifecycle artifact |
| 443 | `amc fleet overview` | One-shot executive fleet summary with verdict, coverage, drift, and next actions |
| 444 | `amc fleet policy` | Fleet governance policy operations |
| 445 | `amc fleet policy apply` | Apply a governance policy to all fleet agents or one environment |
| 446 | `amc fleet policy list` | List effective fleet governance policies |
| 447 | `amc fleet report` | Generate fleet maturity report (md) or fleet compliance report (pdf) |
| 448 | `amc fleet score` | Score multiple agents in one run with fleet-wide aggregates, weak-link detection, and pairwise comparison |
| 449 | `amc fleet slo` | Fleet governance SLO operations |
| 450 | `amc fleet slo define` | Define a fleet SLO, e.g. "95% of production agents must score L3+ on dimension 2" |
| 451 | `amc fleet slo list` | List fleet SLO definitions |
| 452 | `amc fleet slo status` | Show fleet SLO compliance status |
| 453 | `amc fleet status` | Show fleet overview (agent count, average score, health) |
| 454 | `amc fleet tag` | Tag an agent with an environment |
| 455 | `amc fleet trust-add-edge` | Add a delegation edge (orchestrator → worker) |
| 456 | `amc fleet trust-edges` | List all delegation edges |
| 457 | `amc fleet trust-graph` | Render delegation trust graph as Mermaid, DOT, or JSON |
| 458 | `amc fleet trust-init` | Initialize trust composition config |
| 459 | `amc fleet trust-mode` | Set trust inheritance policy mode |
| 460 | `amc fleet trust-receipts` | Verify cross-agent receipt chains |
| 461 | `amc fleet trust-remove-edge` | Remove a delegation edge |
| 462 | `amc fleet trust-report` | Generate trust composition report across fleet |
| 463 | `amc forecast` | Deterministic evidence-gated forecasting and planning |
| 464 | `amc forecast init` | Create and sign forecast policy |
| 465 | `amc forecast latest` | Render latest forecast for scope |
| 466 | `amc forecast policy` | Forecast policy operations |
| 467 | `amc forecast policy apply` | Apply and sign forecast policy from file |
| 468 | `amc forecast policy default` | Print default forecast policy JSON |
| 469 | `amc forecast print-policy` | Print effective forecast policy |
| 470 | `amc forecast refresh` | Refresh forecast snapshot for scope |
| 471 | `amc forecast scheduler` | Forecast renewal scheduler controls |
| 472 | `amc forecast scheduler disable` | Disable forecast scheduler |
| 473 | `amc forecast scheduler enable` | Enable forecast scheduler |
| 474 | `amc forecast scheduler run-now` | Run scheduler refresh immediately |
| 475 | `amc forecast scheduler status` | Show scheduler status |
| 476 | `amc forecast verify` | Verify forecast policy signature |
| 477 | `amc fp-cost` | Show false positive cost summary |
| 478 | `amc fp-list` | List false positive reports |
| 479 | `amc fp-resolve` | Resolve a false positive report |
| 480 | `amc fp-submit` | Submit a false positive report for an assurance scenario |
| 481 | `amc fp-tuning-report` | Generate false positive tuning report with recommendations |
| 482 | `amc framework-guide` | Framework-specific governance guidance |
| 483 | `amc freeze` | Execution freeze status and controls |
| 484 | `amc freeze lift` | - |
| 485 | `amc freeze status` | - |
| 486 | `amc gate` | Evaluate a run bundle against a gate policy |
| 487 | `amc gateway` | AMC universal LLM proxy gateway |
| 488 | `amc gateway bind-agent` | Bind a gateway route prefix to an agent ID for deterministic attribution |
| 489 | `amc gateway init` | Create and sign .amc/gateway.yaml |
| 490 | `amc gateway start` | Start local reverse-proxy gateway and signed evidence capture |
| 491 | `amc gateway status` | Check gateway reachability and route URLs |
| 492 | `amc gateway verify-config` | Verify .amc/gateway.yaml signature |
| 493 | `amc glossary` | Domain terminology management |
| 494 | `amc glossary define` | Define a glossary term |
| 495 | `amc glossary lookup` | Look up a glossary term |
| 496 | `amc governance-drift` | Detect governance drift for an agent |
| 497 | `amc governor` | Autonomy Governor checks |
| 498 | `amc governor check` | Evaluate whether an action is allowed now (simulate vs execute) |
| 499 | `amc governor confidence-check` | Check if action is allowed given confidence-adjusted maturity |
| 500 | `amc governor explain` | Explain policy requirements for an action class |
| 501 | `amc governor report` | Render matrix of current SIMULATE/EXECUTE allowance per ActionClass |
| 502 | `amc governor-override` | Activate an emergency governance override with TTL |
| 503 | `amc governor-override-alerts` | Show alerts for active/expired overrides |
| 504 | `amc guard` | Guard check proposed output from stdin |
| 505 | `amc guardrails` | Signed runtime guardrail controls |
| 506 | `amc guardrails disable` | Remove an additive guardrail request without weakening signed policy |
| 507 | `amc guardrails enable` | Request a signed, runtime-bound guardrail |
| 508 | `amc guardrails list` | List signed requested state and effective runtime guardrail bindings |
| 509 | `amc guardrails profile` | Apply bound controls from a signed additive profile |
| 510 | `amc guide` | Generate personalized improvement guide with exportable agent instructions |
| 511 | `amc help` | Show help for a command (for example: amc help run) |
| 512 | `amc history` | List diagnostic run history |
| 513 | `amc host` | Multi-workspace host mode operations |
| 514 | `amc host bootstrap` | Bootstrap host admin + default workspace from secret files |
| 515 | `amc host init` | Initialize host metadata database |
| 516 | `amc host list` | List host users and workspaces |
| 517 | `amc host membership` | Host membership management |
| 518 | `amc host membership grant` | - |
| 519 | `amc host membership revoke` | - |
| 520 | `amc host migrate` | Migrate an existing single-workspace AMC directory into host mode |
| 521 | `amc host user` | Host user management |
| 522 | `amc host user add` | - |
| 523 | `amc host user disable` | - |
| 524 | `amc host workspace` | Host workspace lifecycle |
| 525 | `amc host workspace create` | - |
| 526 | `amc host workspace delete` | - |
| 527 | `amc host workspace purge` | - |
| 528 | `amc identity` | Enterprise identity (OIDC/SAML) configuration |
| 529 | `amc identity init` | Create and sign host-level identity.yaml |
| 530 | `amc identity mapping` | Signed group-to-role mapping rules |
| 531 | `amc identity mapping add` | Add a group mapping rule |
| 532 | `amc identity provider` | Identity provider management |
| 533 | `amc identity provider add` | Add an identity provider |
| 534 | `amc identity verify` | Verify identity.yaml signature |
| 535 | `amc import` | Import neutral traces, runs, workflow graphs, configs, memory, evals, and benchmarks |
| 536 | `amc imports` | List, inspect, and roll back neutral import runs |
| 537 | `amc imports list` | List recent neutral import runs |
| 538 | `amc imports rollback` | Remove files written by a neutral import run |
| 539 | `amc imports show` | Inspect a neutral import manifest |
| 540 | `amc improve` | Guided improvement — shows what to fix next based on your current score |
| 541 | `amc incident` | Incident tracking and response operations |
| 542 | `amc incident close` | Close an incident with a resolution summary |
| 543 | `amc incident create` | Create a manual incident |
| 544 | `amc incident link` | Link evidence to an incident |
| 545 | `amc incident list` | List incidents for an agent |
| 546 | `amc incident show` | Show incident details |
| 547 | `amc incidents` | Incident operations and dispatch workflows |
| 548 | `amc incidents alert` | Dispatch INCIDENT_CREATED to configured integration channels |
| 549 | `amc incidents help` | Show incident-focused command groups |
| 550 | `amc indices` | Compute deterministic failure-risk indices |
| 551 | `amc indices fleet` | Compute failure-risk indices across fleet |
| 552 | `amc ingest` | Ingest external logs/transcripts as SELF_REPORTED evidence |
| 553 | `amc init` | Initialize .amc workspace |
| 554 | `amc insider-alerts` | Show insider risk alerts |
| 555 | `amc insider-risk-report` | Generate insider risk analytics report |
| 556 | `amc insider-risk-scores` | Show insider risk scores by actor |
| 557 | `amc integrate` | Generate integration scaffold for a framework |
| 558 | `amc integrate-list` | List available integration frameworks |
| 559 | `amc integrations` | Integration hub operations |
| 560 | `amc integrations catalog` | List available integrations |
| 561 | `amc integrations dispatch` | Dispatch a deterministic integration event |
| 562 | `amc integrations export-journal` | Export integration delivery journal (receipts + dead letters) |
| 563 | `amc integrations init` | Create and sign integrations.yaml with vault-backed secret refs |
| 564 | `amc integrations setup` | Generate integration config files |
| 565 | `amc integrations status` | Show integration channels and routing |
| 566 | `amc integrations test` | Dispatch deterministic test event to an integration channel |
| 567 | `amc integrations verify` | Verify integrations config signature |
| 568 | `amc inventory` | AI asset inventory — discover and catalog AI agents, models, and tools |
| 569 | `amc inventory list` | List AI assets (alias for 'inventory scan') |
| 570 | `amc inventory scan` | Scan workspace for AI assets (agents, models, configs, API keys) |
| 571 | `amc key-custody-modes` | List available key custody modes and their configurations |
| 572 | `amc lab-compare` | Compare two lab experiments |
| 573 | `amc lab-create` | Create a new lab experiment |
| 574 | `amc lab-list` | List all lab experiments |
| 575 | `amc lab-report` | Generate a lab experiment report |
| 576 | `amc lab-simulate` | Simulate running all probes for an experiment |
| 577 | `amc lab-templates` | List available experiment templates |
| 578 | `amc leaderboard` | Benchmark leaderboard — compare agent maturity scores |
| 579 | `amc leaderboard export` | Export leaderboard as JSON/HTML for public sharing |
| 580 | `amc leaderboard public-export` | Build an anonymized public leaderboard dataset bundle |
| 581 | `amc leaderboard show` | Show fleet-wide maturity leaderboard |
| 582 | `amc learn` | Education flow for a specific maturity question |
| 583 | `amc lease` | Issue/verify/revoke short-lived agent leases |
| 584 | `amc lease issue` | - |
| 585 | `amc lease revoke` | - |
| 586 | `amc lease verify` | - |
| 587 | `amc legal-hold` | Issue or manage legal holds |
| 588 | `amc lessons-list` | List lessons learned from corrections |
| 589 | `amc lessons-promote` | Promote a correction to a reusable lesson |
| 590 | `amc lifecycle` | Agent lifecycle responsibility and governance mapping |
| 591 | `amc lifecycle advance` | Advance lifecycle stage after governance gate confirmation |
| 592 | `amc lifecycle status` | Show lifecycle stage, accountability matrix, governance gates, and transition trail |
| 593 | `amc lineage-claim` | Show full governance lineage for a specific claim |
| 594 | `amc lineage-init` | Initialize governance lineage tables |
| 595 | `amc lineage-policy-intents` | List all policy change intents for an agent |
| 596 | `amc lineage-report` | Generate governance lineage report |
| 597 | `amc lint` | Lint agent configuration files for schema compliance, anti-patterns, and best practices |
| 598 | `amc lint rules` | List all available lint rules |
| 599 | `amc lite-score` | Lite scoring mode for non-agent LLMs / chatbots — simplified assessment without agentic features |
| 600 | `amc logs` | Print latest AMC Studio logs |
| 601 | `amc loop` | Continuous self-serve maturity loop |
| 602 | `amc loop init` | Initialize recurring loop config |
| 603 | `amc loop plan` | Print recurring loop plan |
| 604 | `amc loop run` | Run recurring diagnostic + assurance + dashboard + snapshot |
| 605 | `amc loop schedule` | Print OS scheduler config (no automatic installation) |
| 606 | `amc maintenance` | Operational maintenance operations |
| 607 | `amc maintenance prune-cache` | Prune dashboard/console/transform cache artifacts |
| 608 | `amc maintenance reindex` | Ensure operational SQLite indexes |
| 609 | `amc maintenance rotate-logs` | Rotate Studio logs based on ops policy |
| 610 | `amc maintenance stats` | Show DB/blob/archive/cache operational stats |
| 611 | `amc maintenance vacuum` | Run SQLite VACUUM + ANALYZE |
| 612 | `amc marketplace` | AMC Pack Marketplace — browse, install, rate community packs |
| 613 | `amc marketplace deprecate` | Deprecate a pack |
| 614 | `amc marketplace featured` | Show featured packs |
| 615 | `amc marketplace info` | Show details for a specific pack |
| 616 | `amc marketplace install` | Install a pack from the marketplace |
| 617 | `amc marketplace list` | List installed packs |
| 618 | `amc marketplace rate` | Rate a pack |
| 619 | `amc marketplace search` | Search marketplace for packs |
| 620 | `amc marketplace undeprecate` | Remove deprecation from a pack |
| 621 | `amc marketplace uninstall` | Uninstall a pack |
| 622 | `amc mcp` | AMC Model Context Protocol (MCP) server for AI coding assistants |
| 623 | `amc mcp config` | Print MCP configuration snippets for supported AI coding assistants |
| 624 | `amc mcp list-tools` | List all tools exposed by the AMC MCP server |
| 625 | `amc mcp serve` | Start the AMC MCP server (stdio transport for IDE integration) |
| 626 | `amc mechanic` | Mechanic Workbench (targets, plans, simulation) |
| 627 | `amc mechanic export` | Export latest gap analysis as reward functions, DSPy targets, or fine-tune recipes |
| 628 | `amc mechanic gap` | - |
| 629 | `amc mechanic init` | - |
| 630 | `amc mechanic plan` | Create, diff, approve, and execute upgrade plans |
| 631 | `amc mechanic plan create` | - |
| 632 | `amc mechanic plan diff` | - |
| 633 | `amc mechanic plan execute` | - |
| 634 | `amc mechanic plan request-approval` | - |
| 635 | `amc mechanic plan show` | - |
| 636 | `amc mechanic profile` | Apply one-click signed target profiles |
| 637 | `amc mechanic profile apply` | - |
| 638 | `amc mechanic profile list` | - |
| 639 | `amc mechanic profile verify` | - |
| 640 | `amc mechanic rca` | Generate fixer root-cause reports from trace failure indexes |
| 641 | `amc mechanic rca list` | List generated fixer RCA reports |
| 642 | `amc mechanic rca run` | Classify a failed run and create regression-preserving fix proposals |
| 643 | `amc mechanic rca show` | Inspect a fixer RCA report |
| 644 | `amc mechanic simulate` | - |
| 645 | `amc mechanic simulations` | Show latest signed simulation artifact |
| 646 | `amc mechanic targets` | Manage signed equalizer targets |
| 647 | `amc mechanic targets apply` | - |
| 648 | `amc mechanic targets init` | - |
| 649 | `amc mechanic targets print` | - |
| 650 | `amc mechanic targets set` | - |
| 651 | `amc mechanic targets verify` | - |
| 652 | `amc mechanic tuning` | Manage signed mechanic tuning intent |
| 653 | `amc mechanic tuning apply` | - |
| 654 | `amc mechanic tuning init` | - |
| 655 | `amc mechanic tuning print` | - |
| 656 | `amc mechanic tuning set` | - |
| 657 | `amc mechanic tuning verify` | - |
| 658 | `amc mechanic verify` | Verify mechanic signatures and artifacts |
| 659 | `amc memory` | Memory maturity assessment and management |
| 660 | `amc memory assess` | Full memory maturity assessment |
| 661 | `amc memory retrieve` | Retrieve active reasoning memory for a consumer |
| 662 | `amc memory show` | Show one reasoning memory item |
| 663 | `amc memory writeback` | Write governed reasoning memory from an EpisodeRecord |
| 664 | `amc memory-advisories` | Show advisories from correction memory for prompt injection |
| 665 | `amc memory-expire` | Expire stale lessons past their TTL |
| 666 | `amc memory-extract` | Extract lessons from verified effective corrections |
| 667 | `amc memory-report` | Generate correction memory report |
| 668 | `amc meta-confidence` | Report confidence in the maturity score itself |
| 669 | `amc methodology` | Print the public AMC scoring methodology manifest and hash |
| 670 | `amc metrics` | Prometheus metrics endpoint helpers |
| 671 | `amc metrics status` | Show configured metrics endpoint bind/port |
| 672 | `amc micro-canary-alerts` | Show active micro-canary alerts |
| 673 | `amc micro-canary-report` | Generate micro-canary status report |
| 674 | `amc micro-canary-run` | Run all micro-canary probes immediately |
| 675 | `amc mirofish` | Agent behavior simulation framework — flight simulator for AI agents |
| 676 | `amc mirofish compare` | Side-by-side comparison of two scenarios |
| 677 | `amc mirofish create` | Interactive scenario builder |
| 678 | `amc mirofish list` | List available built-in scenarios |
| 679 | `amc mirofish run` | Run a Monte Carlo simulation with a scenario |
| 680 | `amc mirofish stress` | Find governance breaking points for a scenario |
| 681 | `amc mode` | Switch CLI role mode |
| 682 | `amc mode agent` | Switch to agent mode (read-only / self-check commands) |
| 683 | `amc mode owner` | Switch to owner mode (configuration + signing allowed) |
| 684 | `amc monitor` | Continuous production monitoring — real-time scoring, drift detection, and alerting |
| 685 | `amc monitor check` | One-shot trust drift analysis (check for degradation without running continuously) |
| 686 | `amc monitor events` | Show recent monitoring events |
| 687 | `amc monitor live` | Start real-time monitoring with live assurance checks on incoming traces |
| 688 | `amc monitor metrics` | Get metrics for a specific agent |
| 689 | `amc monitor start` | Start continuous monitoring: scores agent at intervals, detects drift, sends alerts on degradation |
| 690 | `amc monitor status` | Show monitoring status for all agents |
| 691 | `amc notary` | AMC Notary signing boundary operations |
| 692 | `amc notary attest` | Generate signed notary runtime attestation bundle (.amcattest) |
| 693 | `amc notary init` | Initialize AMC Notary config and signing backend |
| 694 | `amc notary log-verify` | Verify notary append-only signing log + seal signature |
| 695 | `amc notary pubkey` | Print notary public key and fingerprint |
| 696 | `amc notary sign` | Sign a payload file using Notary (admin utility) |
| 697 | `amc notary start` | Start AMC Notary service (foreground) |
| 698 | `amc notary status` | Show notary backend and log status |
| 699 | `amc notary verify-attest` | Verify a .amcattest bundle offline |
| 700 | `amc observe` | Observability — timeline, anomaly detection, and tracing |
| 701 | `amc observe anomalies` | Detect observability anomalies (evidence rate drops, trust regressions, score volatility) |
| 702 | `amc observe timeline` | Show agent evidence timeline with score progression |
| 703 | `amc openapi-generate` | Generate live OpenAPI spec (Studio + Bridge + Gateway) |
| 704 | `amc operator-dashboard` | Generate operator dashboard showing why questions are capped and how to unlock |
| 705 | `amc ops` | Operational hardening policy controls |
| 706 | `amc ops backpressure` | Show backpressure pipeline health |
| 707 | `amc ops circuit-breaker-init` | Initialize circuit breaker policy |
| 708 | `amc ops circuit-breaker-reset` | Reset all circuit breakers |
| 709 | `amc ops circuit-breaker-status` | Show circuit breaker status |
| 710 | `amc ops dead-letters` | Show dead letter queue |
| 711 | `amc ops init` | Create and sign .amc/ops-policy.yaml |
| 712 | `amc ops latency` | Show latency accounting report |
| 713 | `amc ops mode` | Show or set degradation mode |
| 714 | `amc ops print` | Print effective ops policy |
| 715 | `amc ops slo` | Show governance SLO dashboard |
| 716 | `amc ops verify` | Verify ops-policy signature |
| 717 | `amc org` | Org graph and real-time comparative scorecards |
| 718 | `amc org add` | - |
| 719 | `amc org add node` | - |
| 720 | `amc org assign` | - |
| 721 | `amc org commit` | - |
| 722 | `amc org community` | Community/platform governance scoring |
| 723 | `amc org community init` | - |
| 724 | `amc org community score` | - |
| 725 | `amc org compare` | - |
| 726 | `amc org init` | - |
| 727 | `amc org inspect` | - |
| 728 | `amc org learn` | - |
| 729 | `amc org own` | - |
| 730 | `amc org report` | - |
| 731 | `amc org roles` | List the canonical 70 AMC org roles |
| 732 | `amc org run` | Run the advanced 70-role org lifecycle loop with isolated role workspaces |
| 733 | `amc org runs` | List org lifecycle runs |
| 734 | `amc org score` | - |
| 735 | `amc org unassign` | - |
| 736 | `amc org verify` | Verify signed org.yaml |
| 737 | `amc outcomes` | Outcome contracts, value signals, and reports |
| 738 | `amc outcomes attest` | Record manual attested outcome signal |
| 739 | `amc outcomes diff` | Diff two outcome reports |
| 740 | `amc outcomes init` | Create and sign outcome contract |
| 741 | `amc outcomes report` | Generate outcomes report (agent) or fleet outcomes report |
| 742 | `amc outcomes verify` | Verify outcome contract signature |
| 743 | `amc overhead-profile` | Set the overhead mode profile (STRICT, BALANCED, LEAN) |
| 744 | `amc overhead-report` | Generate per-feature overhead accounting report |
| 745 | `amc oversight` | Human oversight quality assessment |
| 746 | `amc oversight assess` | Assess human oversight quality |
| 747 | `amc own` | Ownership flow for top maturity gaps |
| 748 | `amc pack` | Community assurance pack registry — NPM-style package management |
| 749 | `amc pack info` | Show detailed information about a pack |
| 750 | `amc pack init` | Initialize a new pack in <name>/ or an explicit --dir |
| 751 | `amc pack install` | Install a community assurance pack |
| 752 | `amc pack list` | List installed packs |
| 753 | `amc pack publish` | Publish a pack to the registry |
| 754 | `amc pack registry` | Pack registry management |
| 755 | `amc pack registry init` | Initialize local pack registry |
| 756 | `amc pack registry serve` | Start a local pack registry server |
| 757 | `amc pack search` | Search for packs in the registry |
| 758 | `amc pack test` | Test a local pack directory; defaults to cwd and auto-detects one child pack |
| 759 | `amc pack uninstall` | Uninstall a pack |
| 760 | `amc pair` | LAN pairing code operations |
| 761 | `amc pair create` | Create one-time pairing code (LAN login pairing or agent bridge pairing) |
| 762 | `amc pair redeem` | Redeem pairing code for a lease token file |
| 763 | `amc passport` | Agent Passport (shareable maturity credential) |
| 764 | `amc passport badge` | Print deterministic single-line badge from latest cache |
| 765 | `amc passport capabilities-add` | Add capability declaration to agent passport |
| 766 | `amc passport compare` | Compare two agents by passport maturity dimensions |
| 767 | `amc passport create` | Create deterministic signed .amcpass artifact |
| 768 | `amc passport export-latest` | Export latest passport for a scope to .amcpass |
| 769 | `amc passport init` | Create and sign .amc/passport/policy.yaml |
| 770 | `amc passport issue-token` | Issue an AMC Trust Token for an agent |
| 771 | `amc passport link` | Link agent passport to external platform identity |
| 772 | `amc passport policy` | Passport policy operations |
| 773 | `amc passport policy apply` | Apply passport policy from JSON/YAML file |
| 774 | `amc passport policy print` | Print effective passport policy |
| 775 | `amc passport search` | Search agents by capability and minimum maturity level |
| 776 | `amc passport share` | Generate shareable passport material |
| 777 | `amc passport show` | Show .amcpass as JSON or single-line badge |
| 778 | `amc passport translate-score` | Translate trust scores between scoring systems |
| 779 | `amc passport verify` | Verify .amcpass artifact offline |
| 780 | `amc passport verify-policy` | Verify signed passport policy |
| 781 | `amc passport verify-token` | Verify an AMC Trust Token (pass JSON string) |
| 782 | `amc playground` | Interactive scenario runner |
| 783 | `amc playground list` | List available scenarios |
| 784 | `amc playground run` | Run all demo scenarios |
| 785 | `amc plugin` | Signed content-only extension marketplace |
| 786 | `amc plugin execute` | Execute approved plugin install/upgrade/remove request |
| 787 | `amc plugin init` | Initialize signed plugin workspace files |
| 788 | `amc plugin install` | Request plugin install (requires SECURITY dual-control approval) |
| 789 | `amc plugin keygen` | Generate plugin publisher keypair |
| 790 | `amc plugin limits` | Show current plugin sandbox resource limits |
| 791 | `amc plugin list` | List installed plugins and verification status |
| 792 | `amc plugin pack` | Create signed .amcplug package from a plugin folder |
| 793 | `amc plugin print` | Print plugin manifest summary |
| 794 | `amc plugin registries` | List signed workspace registry configuration |
| 795 | `amc plugin registries-apply` | Apply and sign workspace registries.yaml from JSON or YAML file |
| 796 | `amc plugin registry` | Manage plugin registries |
| 797 | `amc plugin registry init` | Initialize local signed plugin registry directory |
| 798 | `amc plugin registry publish` | Publish plugin package into registry and re-sign index |
| 799 | `amc plugin registry serve` | Serve plugin registry over local HTTP |
| 800 | `amc plugin registry verify` | Verify registry signature and package hashes |
| 801 | `amc plugin registry-fingerprint` | Compute registry public key fingerprint |
| 802 | `amc plugin remove` | Request plugin removal (requires SECURITY dual-control approval) |
| 803 | `amc plugin search` | Search a plugin registry by id/fingerprint |
| 804 | `amc plugin upgrade` | Request plugin upgrade (requires SECURITY dual-control approval) |
| 805 | `amc plugin verify` | Verify plugin package signature + artifact hashes |
| 806 | `amc plugin workspace-verify` | Verify workspace plugin signatures/integrity |
| 807 | `amc policy` | Policy-as-code operations |
| 808 | `amc policy action` | Signed autonomy action policy |
| 809 | `amc policy action init` | Create and sign .amc/action-policy.yaml |
| 810 | `amc policy action logic` | Compose existing Action Policy evidence requirements |
| 811 | `amc policy action logic apply` | Apply evidence logic after exact confirmation |
| 812 | `amc policy action logic compile` | Preview a deterministic evidence-logic change without writing |
| 813 | `amc policy action logic show` | Show declared evidence gates and effective logic |
| 814 | `amc policy action verify` | Verify action policy signature |
| 815 | `amc policy approval` | Signed dual-control approval policy |
| 816 | `amc policy approval init` | Create and sign .amc/approval-policy.yaml |
| 817 | `amc policy approval verify` | Verify approval-policy signature |
| 818 | `amc policy controls` | Show one verified Scope / When / Then projection of existing controls |
| 819 | `amc policy pack` | Policy packs by archetype and risk tier |
| 820 | `amc policy pack apply` | Apply policy pack and sign updated configs/targets |
| 821 | `amc policy pack describe` | Describe policy pack contents |
| 822 | `amc policy pack diff` | Show deterministic diff for applying a policy pack |
| 823 | `amc policy pack list` | List built-in policy packs |
| 824 | `amc policy scope` | Compile reusable action-class scopes into existing signed policies |
| 825 | `amc policy scope apply` | Apply a scope preview after exact compile-ID confirmation |
| 826 | `amc policy scope compile` | Preview a deterministic selected-rule merge without writing |
| 827 | `amc policy scope list` | List immutable AMC action-class scope templates |
| 828 | `amc policy simulate` | Simulate one projected control through its production evaluator without recording |
| 829 | `amc policy-canary-report` | Generate canary mode report for an agent |
| 830 | `amc policy-canary-start` | Start policy canary mode (observation-only) |
| 831 | `amc policy-debt-add` | Register a temporary policy waiver (debt) |
| 832 | `amc policy-debt-list` | List active policy debt entries |
| 833 | `amc product` | Product operations: routing, autonomy, metering, workflows |
| 834 | `amc product autonomy` | Decide autonomy level for an agent |
| 835 | `amc product features` | List product features |
| 836 | `amc product features-recommended` | Show top recommended product features |
| 837 | `amc product loop-detect` | Detect infinite loops in agent behavior |
| 838 | `amc product metering` | Show metering and billing for an agent |
| 839 | `amc product plan` | Generate an execution plan for a goal |
| 840 | `amc product retry` | Execute a command with retry logic |
| 841 | `amc product route` | Route a task to the best model/provider |
| 842 | `amc product workflow` | Workflow management |
| 843 | `amc product workflow create` | Create a new workflow |
| 844 | `amc prompt` | Northstar prompt policy + pack operations |
| 845 | `amc prompt init` | Create and sign .amc/prompt/policy.yaml |
| 846 | `amc prompt pack` | Prompt pack artifact operations |
| 847 | `amc prompt pack build` | Build and sign .amcprompt for an agent |
| 848 | `amc prompt pack diff` | Diff latest prompt pack against previous snapshot |
| 849 | `amc prompt pack show` | Show provider-specific enforced system prompt |
| 850 | `amc prompt pack verify` | Verify .amcprompt signature and lint signature |
| 851 | `amc prompt policy` | Prompt policy operations |
| 852 | `amc prompt policy apply` | Apply prompt policy from YAML file and sign |
| 853 | `amc prompt policy print` | Print prompt policy |
| 854 | `amc prompt scheduler` | Prompt pack recurrence scheduler |
| 855 | `amc prompt scheduler disable` | Disable prompt scheduler |
| 856 | `amc prompt scheduler enable` | Enable prompt scheduler |
| 857 | `amc prompt scheduler run-now` | Run prompt scheduler now for one agent or all |
| 858 | `amc prompt scheduler status` | Show prompt scheduler status |
| 859 | `amc prompt status` | List per-agent prompt pack status |
| 860 | `amc prompt verify` | Verify prompt policy, pack, lint and scheduler signatures |
| 861 | `amc proof` | Domain Proof Lane source-to-rule proof checks |
| 862 | `amc proof check` | Check a claim against a declared source-to-rule manifest and emit an amcproof artifact |
| 863 | `amc provider` | Provider template operations |
| 864 | `amc provider add` | Assign or update provider template for an agent |
| 865 | `amc provider list` | List provider templates |
| 866 | `amc python-sdk` | Generate the Python SDK package for AMC Bridge API |
| 867 | `amc quality-report` | Show quality report |
| 868 | `amc quickscore` | Full default interactive diagnostic — or use --rapid for 5-question express, --auto for ledger evidence |
| 869 | `amc quickstart` | 2-minute quickstart with Quick Score assessment |
| 870 | `amc rate` | Rate agent run quality (thumbs up/down) |
| 871 | `amc receipts-chain` | Show full delegation chain for a receipt |
| 872 | `amc redaction-test` | Run privacy redaction tests against built-in rules |
| 873 | `amc redteam` | Run red-team attack simulations against a target agent |
| 874 | `amc redteam attack` | Run attack plugins (prompt-injection, data-exfiltration, privilege-escalation, model-manipulation, denial-of-service) |
| 875 | `amc redteam attack-list` | List available attack plugins |
| 876 | `amc redteam plugins` | List available attack plugins (assurance packs) |
| 877 | `amc redteam run` | Execute red-team plugins with chosen attack strategies and generate a vulnerability report |
| 878 | `amc redteam strategies` | List available attack strategies |
| 879 | `amc release` | Deterministic release engineering and offline verification |
| 880 | `amc release init` | Initialize AMC release signing keypair |
| 881 | `amc release licenses` | Generate dependency license inventory |
| 882 | `amc release pack` | Build a signed deterministic .amcrelease bundle |
| 883 | `amc release print` | Print release bundle manifest summary |
| 884 | `amc release provenance` | Generate AMC provenance record |
| 885 | `amc release sbom` | Generate deterministic CycloneDX SBOM |
| 886 | `amc release scan` | Run strict secret scan on a .amcrelease bundle |
| 887 | `amc release verify` | Verify a .amcrelease bundle offline |
| 888 | `amc report` | Render report for run ID, saved alias, prefix, or 'latest' |
| 889 | `amc residency-policy` | Create or list data residency policies |
| 890 | `amc residency-report` | Generate data residency compliance report for a tenant |
| 891 | `amc resource` | Govern prompts, tools, memory, policies, routes, and other agent-defining resources |
| 892 | `amc resource apply` | Accept current resources as the new signed manifest; dry-run unless --yes is set |
| 893 | `amc resource contract` | Show the AMC-native governed resource lifecycle contract |
| 894 | `amc resource diff` | Diff an Enforce resource manifest against the current workspace |
| 895 | `amc resource evaluate` | Evaluate a resource proposal against Enforce gates |
| 896 | `amc resource get` | Inspect one resource in an Enforce resource manifest |
| 897 | `amc resource history` | Show signed Enforce resource manifests, snapshots, and receipts |
| 898 | `amc resource list` | List resources in an Enforce resource manifest |
| 899 | `amc resource propose` | Create a dry-run resource change proposal from the latest manifest to current workspace state |
| 900 | `amc resource restore` | Restore resources from an Enforce snapshot; dry-run unless --apply is set |
| 901 | `amc resource rollback` | Roll back to the signed previous version, or an explicit canonical snapshot |
| 902 | `amc resource snapshot` | Write the current Enforce resource manifest |
| 903 | `amc resource status` | Show the signed active, previous, rollback, drift, and integrity state |
| 904 | `amc resource validate` | Validate governed resource changes before accepting them |
| 905 | `amc retention` | Retention/archive payload lifecycle operations |
| 906 | `amc retention run` | Run archival + payload prune lifecycle |
| 907 | `amc retention status` | Show retention/archive status |
| 908 | `amc retention verify` | Verify archive manifests/signatures and ledger continuity |
| 909 | `amc role-presets` | List available dashboard role presets |
| 910 | `amc rollback-create` | Create a rollback pack from the current policy file |
| 911 | `amc run` | Full assessment — Score + Shield + Enforce + Vault + Watch + Comply + Fleet + Passport in one command |
| 912 | `amc run-alias` | Name diagnostic runs for report and history workflows |
| 913 | `amc run-alias list` | List diagnostic run aliases for the active agent |
| 914 | `amc run-alias remove` | Remove a diagnostic run alias |
| 915 | `amc run-alias set` | Assign a reusable alias to a diagnostic run |
| 916 | `amc runtime` | Runtime run manager for connected agents |
| 917 | `amc runtime cancel` | Cancel a runtime run cleanly |
| 918 | `amc runtime complete` | Complete a runtime run |
| 919 | `amc runtime create` | Create a persisted connected-agent runtime run |
| 920 | `amc runtime degrade` | Mark a runtime run degraded |
| 921 | `amc runtime event` | Append an event to a persisted runtime run |
| 922 | `amc runtime export` | Export runtime run events as JSON or JSONL |
| 923 | `amc runtime inspect` | Inspect a runtime run and its event stream |
| 924 | `amc runtime list` | List persisted runtime runs |
| 925 | `amc runtime resume` | Resume a running or degraded runtime run from persisted state |
| 926 | `amc runtime status` | Show persisted runtime run-manager status |
| 927 | `amc sandbox` | Hardened sandbox execution |
| 928 | `amc sandbox run` | Run agent command in hardened Docker sandbox |
| 929 | `amc scan` | Zero-integration agent assessment scanner |
| 930 | `amc scan model-scan` | Scan ML model files for security threats (malicious code, backdoors, supply chain attacks) |
| 931 | `amc scim` | SCIM token management |
| 932 | `amc scim init` | Enable SCIM provisioning and optionally create an initial bearer token |
| 933 | `amc scim token` | SCIM bearer token operations |
| 934 | `amc scim token create` | Create a SCIM bearer token and store hash in host vault |
| 935 | `amc score` | Maturity scoring, adversarial testing, and evidence collection |
| 936 | `amc score a2a-protocol` | Score agent-to-agent protocol maturity: card completeness, lifecycle, auth, format, errors, discovery |
| 937 | `amc score adversarial` | Test gaming resistance of scoring |
| 938 | `amc score alignment-index` | Compute composite alignment index |
| 939 | `amc score audit-depth` | Score audit trail depth and completeness |
| 940 | `amc score autonomy-duration` | Track time between human checkpoints with domain risk profiles |
| 941 | `amc score behavioral-contract` | Score agent behavioral contract maturity (alignment card, permitted/forbidden actions) |
| 942 | `amc score calibration-gap` | Measure delta between agent self-reported confidence and observed behavior |
| 943 | `amc score collect-evidence` | Collect evidence for scoring an agent |
| 944 | `amc score density-map` | Heatmap of evidence density per question per dimension — reveals blind spots |
| 945 | `amc score distributed-agents` | Score distributed multi-agent execution: partitions, sync, failover, consensus, load, observability |
| 946 | `amc score eu-ai-act` | Score EU AI Act compliance maturity (Art. 9-17, GPAI systemic risk) |
| 947 | `amc score evidence-conflict` | Measure internal consistency of evidence — detect conflicting signals |
| 948 | `amc score evidence-coverage` | Show automated vs manual evidence coverage |
| 949 | `amc score evidence-ingest` | Ingest evidence from external systems (openai-evals, langsmith, mlflow, custom) |
| 950 | `amc score factuality` | Score factuality across parametric, retrieval, and grounded dimensions |
| 951 | `amc score fail-secure` | Score fail-secure tool governance (deny-by-default, rate limiting, anomaly detection) |
| 952 | `amc score faithfulness` | Score how well LLM output is grounded in provided context |
| 953 | `amc score formal-spec` | Compute formal maturity score for an agent |
| 954 | `amc score gaming-resistance` | Test whether adversarial evidence injection can inflate scores |
| 955 | `amc score industry-adjust` | Adjust a score using an industry-specific trust model |
| 956 | `amc score industry-benchmark` | Show industry benchmark percentiles |
| 957 | `amc score industry-list` | List all available industry trust models |
| 958 | `amc score interpretability` | Score structural transparency and explainability |
| 959 | `amc score kernel-sandbox` | Score kernel-level sandbox maturity (OS isolation, filesystem/network restrictions) |
| 960 | `amc score lean-profile` | Show lean AMC profile |
| 961 | `amc score level-transition` | Track formal promotion/demotion events with evidence gates |
| 962 | `amc score memory-depth` | Score deep memory infrastructure: backend resilience, compression fidelity, cross-session consistency, TTL, capacity |
| 963 | `amc score memory-integrity` | Score memory correction persistence and poisoning resistance |
| 964 | `amc score mutual-verification` | Score agent-to-agent trust verification (challenge-response) |
| 965 | `amc score operational-independence` | Calculate operational independence score |
| 966 | `amc score output-attestation` | Score output signing and trust metadata for receiving agents |
| 967 | `amc score output-integrity` | Score output integrity maturity (OWASP LLM02, confidence calibration, citation) |
| 968 | `amc score owasp-llm` | Score OWASP LLM Top 10 coverage (all 10 risks) |
| 969 | `amc score pause-quality` | Score quality of agent-initiated pauses |
| 970 | `amc score policy-consistency` | Test policy enforcement consistency across repeated trials (pass^k) |
| 971 | `amc score production-ready` | Run production readiness gate for an agent |
| 972 | `amc score regulatory-readiness` | Compute weighted regulatory readiness score (EU AI Act + ISO + OWASP) |
| 973 | `amc score runtime-identity` | Score runtime execution identity maturity (JIT credentials, user propagation, revocation) |
| 974 | `amc score safety-research` | Run the AI Safety Research evaluation lane — 4-dimension assessment based on frontier safety research |
| 975 | `amc score self-knowledge` | Score prior art self-knowledge maturity (typed attention, trace layer, confidence+citation) |
| 976 | `amc score simulation-lane` | Run the Simulation & Forecast evaluation lane — 5-dimension assessment for simulation/forecast systems |
| 977 | `amc score sleeper-detection` | Detect context-dependent behavioral inconsistencies |
| 978 | `amc score state-portability` | Score agent state portability (vendor-neutral format, serialization, integrity on transfer) |
| 979 | `amc score task-horizon` | Score task-completion time horizon (METR-inspired) |
| 980 | `amc score tier` | Run tiered maturity assessment (quick/standard/deep) |
| 981 | `amc score transparency-log` | Score network transparency log (Merkle tree, inclusion proofs) |
| 982 | `amc sessions` | View and analyze user sessions |
| 983 | `amc sessions list` | List tracked sessions |
| 984 | `amc setup` | Setup wizard for the full-score path and Studio gateway |
| 985 | `amc shell` | Interactive AMC session — natural language + commands |
| 986 | `amc shield` | Threat detection and security scanning |
| 987 | `amc shield analyze` | Run static code analyzer on a skill file |
| 988 | `amc shield analyze-mcp` | Scan an MCP server definition for security risks (score L0–L5) |
| 989 | `amc shield analyze-runtime` | Analyze a proposed runtime agent action through the Shield trust pipeline |
| 990 | `amc shield confirm` | Controlled exploit confirmation with strict authorization gates |
| 991 | `amc shield confirm export` | Export a redacted safe proof without exploit instructions |
| 992 | `amc shield confirm proofs` | List safe exploit-confirmation proof artifacts |
| 993 | `amc shield confirm run` | Run authorized safe exploit confirmation from a task JSON file |
| 994 | `amc shield confirm scope-write` | Write a signed exploit-confirmation authorization scope from JSON |
| 995 | `amc shield confirm scopes` | List exploit-confirmation authorization scopes |
| 996 | `amc shield conversation-integrity` | Check conversation integrity for an agent (demo) |
| 997 | `amc shield detect-injection` | Detect prompt injection attempts in text |
| 998 | `amc shield red-team` | Run a quick red team campaign (5 attacks on demo target). Tip: For full red-team suite with strategies, use `amc redteam run` |
| 999 | `amc shield red-team-status` | Show current red team capabilities and attack template count |
| 1000 | `amc shield reputation` | Check reputation score for a tool |
| 1001 | `amc shield sandbox` | Check sandbox configuration for an agent |
| 1002 | `amc shield sanitize` | Sanitize text — strip LLM prompt injection and dangerous AI patterns (not SQL/XSS) |
| 1003 | `amc shield sbom` | Generate software bill of materials from package.json |
| 1004 | `amc shield threat-intel` | Check threat intelligence for an input |
| 1005 | `amc shield trust-pipeline` | Run end-to-end trust pipeline for an agent action |
| 1006 | `amc simulate-bridge` | Run a simulated bridge request for local testing |
| 1007 | `amc snapshot` | Generate Unified Clarity Snapshot markdown |
| 1008 | `amc sso` | SSO setup shortcuts for OIDC and SAML providers |
| 1009 | `amc sso configure` | Configure an OIDC or SAML SSO provider |
| 1010 | `amc standard` | Open Compass Standard schema bundle and validation |
| 1011 | `amc standard generate` | Generate signed Open Compass schema bundle under .amc/standard/ |
| 1012 | `amc standard print` | Print one generated schema |
| 1013 | `amc standard schemas` | List generated schemas with digests |
| 1014 | `amc standard validate` | Validate a JSON file or AMC artifact against a standard schema |
| 1015 | `amc standard verify` | Verify schema bundle signatures and manifest digests |
| 1016 | `amc status` | Show AMC Studio and vault status |
| 1017 | `amc strategy` | Compare inference strategies and govern route changes |
| 1018 | `amc strategy compare` | Compare model/provider strategies with score, cost, latency, risk, and evidence |
| 1019 | `amc strategy list` | List inference strategy comparison runs |
| 1020 | `amc strategy rollback` | Roll back an accepted inference route change |
| 1021 | `amc strategy show` | Inspect an inference strategy comparison run |
| 1022 | `amc studio` | Studio API helpers |
| 1023 | `amc studio healthcheck` | Health/readiness probe for deployment runtime |
| 1024 | `amc studio lan` | LAN mode controls for Compass Console |
| 1025 | `amc studio lan disable` | Disable LAN mode and revert to localhost-only |
| 1026 | `amc studio lan enable` | Enable LAN mode with pairing gate |
| 1027 | `amc studio ping` | Ping local Studio API /health endpoint |
| 1028 | `amc studio start` | Start Studio in foreground (non-interactive, deployment-safe) |
| 1029 | `amc supervise` | Supervise any agent process and inject gateway/proxy routing env vars |
| 1030 | `amc target` | Target profile operations |
| 1031 | `amc target diff` | Diff run against target profile |
| 1032 | `amc target set` | Interactive equalizer wizard |
| 1033 | `amc target verify` | Verify target profile signature |
| 1034 | `amc tenant-isolation-check` | Check tenant isolation between all registered tenants |
| 1035 | `amc tenant-register` | Register a tenant boundary |
| 1036 | `amc ticket` | Execution ticket operations |
| 1037 | `amc ticket issue` | Issue short-lived signed execution ticket |
| 1038 | `amc ticket verify` | Verify signed execution ticket |
| 1039 | `amc tools` | ToolHub tools config |
| 1040 | `amc tools init` | Create and sign .amc/tools.yaml |
| 1041 | `amc tools list` | List allowed ToolHub tools and action classes |
| 1042 | `amc tools verify` | Verify tools.yaml signature |
| 1043 | `amc trace` | Trace explorer — inspect agent execution traces, sessions, and tool calls |
| 1044 | `amc trace failures` | Show top recurring failure clusters mined from trace indexes |
| 1045 | `amc trace index` | List or inspect distilled trace failure indexes |
| 1046 | `amc trace inspect` | Inspect evidence events — show tool calls, decisions, and trust tiers |
| 1047 | `amc trace list` | List recent agent sessions with evidence summary |
| 1048 | `amc trace stats` | Show trace statistics — event counts by type, trust tier, tool usage |
| 1049 | `amc transform` | Transformation OS (4C plans, tracking, attestations) |
| 1050 | `amc transform attest` | - |
| 1051 | `amc transform attest-verify` | - |
| 1052 | `amc transform init` | Initialize signed .amc/transform-map.yaml |
| 1053 | `amc transform map` | Inspect or apply transform map |
| 1054 | `amc transform map apply` | - |
| 1055 | `amc transform map show` | - |
| 1056 | `amc transform plan` | - |
| 1057 | `amc transform report` | - |
| 1058 | `amc transform status` | - |
| 1059 | `amc transform track` | - |
| 1060 | `amc transform verify` | Verify signed transform map |
| 1061 | `amc transparency` | Append-only transparency log operations |
| 1062 | `amc transparency export` | Export transparency bundle |
| 1063 | `amc transparency init` | Initialize append-only transparency log |
| 1064 | `amc transparency merkle` | Merkle transparency root/proof operations |
| 1065 | `amc transparency merkle prove` | Export signed inclusion proof bundle for entry hash |
| 1066 | `amc transparency merkle rebuild` | Rebuild Merkle leaves/roots from transparency log |
| 1067 | `amc transparency merkle root` | Show current Merkle root and history |
| 1068 | `amc transparency merkle verify-proof` | Verify signed inclusion proof bundle |
| 1069 | `amc transparency report` | Generate an Agent Transparency Report — what the agent does, can access, and how trustworthy it is |
| 1070 | `amc transparency tail` | Tail transparency entries |
| 1071 | `amc transparency verify` | Verify transparency chain + seal signature |
| 1072 | `amc transparency verify-bundle` | Verify exported transparency bundle |
| 1073 | `amc trust` | Trust mode and Notary enforcement configuration |
| 1074 | `amc trust enable-notary` | Enable fail-closed NOTARY trust mode |
| 1075 | `amc trust freshness` | Report temporal trust freshness and half-life decay |
| 1076 | `amc trust init` | Create and sign .amc/trust.yaml — sets up the trust mode (SELF/NOTARY) that governs artifact signing |
| 1077 | `amc trust status` | Show trust mode, signature status, and notary health |
| 1078 | `amc truthguard` | Deterministic output truth-constraint validator |
| 1079 | `amc truthguard validate` | Validate structured agent output claims against deterministic truth constraints |
| 1080 | `amc tune` | Mechanic mode tuning wizard |
| 1081 | `amc unknowns` | List known unknowns for an agent's latest diagnostic run |
| 1082 | `amc up` | Start AMC control plane in one command (studio + gateway + bridge) |
| 1083 | `amc upgrade` | Generate upgrade plan |
| 1084 | `amc user` | Multi-user RBAC account management |
| 1085 | `amc user add` | Add a user with RBAC roles |
| 1086 | `amc user init` | Initialize signed users.yaml with first OWNER user |
| 1087 | `amc user list` | List RBAC users |
| 1088 | `amc user revoke` | Revoke a user account |
| 1089 | `amc user role` | Set user roles |
| 1090 | `amc user role set` | Replace roles for a user |
| 1091 | `amc user verify` | Verify users.yaml signature |
| 1092 | `amc value` | Value realization engine (contracts, scoring, ROI) |
| 1093 | `amc value contract` | Value contract operations |
| 1094 | `amc value contract apply` | Apply value contract from YAML/JSON file |
| 1095 | `amc value contract init` | Create and sign value contract template |
| 1096 | `amc value contract print` | Print value contract and signature status |
| 1097 | `amc value contract verify` | Verify value contract signature |
| 1098 | `amc value import` | Import numeric KPI points from CSV (ts,value) |
| 1099 | `amc value ingest` | Ingest value webhook payload JSON |
| 1100 | `amc value init` | Initialize signed value policy, default contract, and scheduler |
| 1101 | `amc value policy` | Value policy operations |
| 1102 | `amc value policy apply` | Apply signed value policy from YAML/JSON file |
| 1103 | `amc value policy default` | Print default value policy JSON |
| 1104 | `amc value policy print` | Print effective value policy JSON |
| 1105 | `amc value report` | Generate signed value report |
| 1106 | `amc value scheduler` | Value scheduler controls |
| 1107 | `amc value scheduler disable` | Disable value scheduler |
| 1108 | `amc value scheduler enable` | Enable value scheduler |
| 1109 | `amc value scheduler run-now` | Run value scheduler now |
| 1110 | `amc value scheduler status` | Show value scheduler status |
| 1111 | `amc value snapshot` | Generate/load latest signed value snapshot |
| 1112 | `amc value verify` | Verify value workspace signatures/artifacts |
| 1113 | `amc value verify-policy` | Verify signed value policy |
| 1114 | `amc vault` | Encrypted key vault operations |
| 1115 | `amc vault classify` | Classify data sensitivity level |
| 1116 | `amc vault dlp` | DLP scanner for PII and secrets |
| 1117 | `amc vault dlp scan` | Scan text for PII and secrets |
| 1118 | `amc vault dsar` | Persistent DSAR (Data Subject Access Request) workflow |
| 1119 | `amc vault dsar complete` | Mark a DSAR request complete and append an audit event |
| 1120 | `amc vault dsar list` | List persistent DSAR requests |
| 1121 | `amc vault dsar status` | Show a persistent DSAR request |
| 1122 | `amc vault dsar submit` | Submit a persistent DSAR request |
| 1123 | `amc vault dsar-status` | Show DSAR (Data Subject Access Request) status |
| 1124 | `amc vault init` | Initialize encrypted vault for signing keys |
| 1125 | `amc vault lock` | Lock vault and clear in-memory private keys |
| 1126 | `amc vault privacy-budget` | Check privacy budget for an agent |
| 1127 | `amc vault rag-guard` | Guard RAG chunks against injection |
| 1128 | `amc vault rotate-keys` | Rotate monitor signing key and append to public key history |
| 1129 | `amc vault scrub` | Scrub metadata from a file |
| 1130 | `amc vault secret-share` | Split a secret into shares using Shamir's Secret Sharing |
| 1131 | `amc vault status` | Show vault status |
| 1132 | `amc vault unlock` | Unlock vault into memory for signing operations |
| 1133 | `amc vault zk-commit` | Create a Pedersen commitment to a value |
| 1134 | `amc vault zk-range-proof` | Create a zero-knowledge range proof that an AMC score meets a threshold |
| 1135 | `amc vault zk-verify` | Verify a ZK range proof (pass JSON as string) |
| 1136 | `amc verify` | Verify integrity across AMC artifacts |
| 1137 | `amc verify all` | Verify trust/policies/plugins/logs/ledger/artifacts in one pass |
| 1138 | `amc vibe-audit` | Run static safety checks for AI-generated code |
| 1139 | `amc watch` | Observability, attestation, and safety testing |
| 1140 | `amc watch alerts` | Show recent alerts for a monitored agent |
| 1141 | `amc watch attest` | Attest an agent output |
| 1142 | `amc watch connect` | Connect to an observability provider (langfuse, helicone, otlp, datadog, webhook) |
| 1143 | `amc watch explain` | Generate explainability packet for an agent run |
| 1144 | `amc watch host-hardening` | Check host hardening status for this AMC deployment |
| 1145 | `amc watch profiler-anomalies` | List detected behavioral anomalies for an agent |
| 1146 | `amc watch profiler-start` | Start behavioral profiling for an agent |
| 1147 | `amc watch profiler-status` | Show behavioral profiler status and any recent anomalies |
| 1148 | `amc watch providers` | Show connected observability providers and trace stats |
| 1149 | `amc watch safety-test` | Run safety tests for an agent |
| 1150 | `amc watch start` | Start continuous production monitoring for an agent |
| 1151 | `amc watch status` | Show all monitored agents and their current state |
| 1152 | `amc whatif` | Equalizer what-if simulator |
| 1153 | `amc whatif equalizer` | - |
| 1154 | `amc whatif targets` | - |
| 1155 | `amc why-capped` | Show why each question is capped at its current level |
| 1156 | `amc wiring-status` | Show production wiring status for all modules (Items 11-16) |
| 1157 | `amc workorder` | Signed work order operations |
| 1158 | `amc workorder create` | Create and sign a work order |
| 1159 | `amc workorder expire` | Expire/revoke a work order |
| 1160 | `amc workorder list` | List work orders for agent |
| 1161 | `amc workorder show` | Show signed work order JSON |
| 1162 | `amc workorder verify` | Verify work order signature |
| 1163 | `amc wrap` | Wrap runtime and capture tamper-evident evidence |
