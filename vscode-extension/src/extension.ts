...
export function activate(context: vscode.ExtensionContext) {
  ...
  const applyQuickFixCommand = vscode.commands.registerCommand('amc.quickfix.apply', async (uri: vscode.Uri, line: number, fix: string) => {
  ...
  async function applyQuickFix(uri: vscode.Uri, line: number, fix: string) {
  ...
  function updateStatusBar(result: AmcResult) {
  ...
  async function findAmcExecutable(workspacePath: string): Promise<string | null> {
  ...
  async function runAmcAnalysis(amcPath: string, filePath: string): Promise<AmcResult> {
    try {
...
class AmcResultsProvider implements vscode.TreeDataProvider<AmcResultItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<AmcResultItem | undefined | null | void> = new vscode.EventEmitter<AmcResultItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<AmcResultItem | undefined | null | void> = this._onDidChangeTreeData.event;

  private results: AmcResult | null = null;

  updateResults(results: AmcResult) {
    this.results = results;
    this._onDidChangeTreeData.fire();
  }

  getTreeItem(element: AmcResultItem): vscode.TreeItem {
    return element;
  }

  getChildren(element?: AmcResultItem): Thenable<AmcResultItem[]> {
    if (!this.results) {
    ...
    if (element.label === 'Question Scores') {
      return Promise.resolve(
        Object.entries(this.results.questionScores).map(([question, score]) =>
          new AmcResultItem(`${question}: ${score.toFixed(1)}`, vscode.TreeItemCollapsibleState.None)
        )
      );
    }
    ...
    if (element.label?.toString().startsWith('Issues')) {
      return Promise.resolve(
        this.results.issues.map(issue =>
          new AmcResultItem(`Line ${issue.line}: ${issue.message}`, vscode.TreeItemCollapsibleState.None)
        )
      );
    }
