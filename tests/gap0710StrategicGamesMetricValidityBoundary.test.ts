...
const DOC = "docs/source-reviews/GAP-0710-multi-agent-strategic-games-metric-validity.md";
const SOURCE = "https://arxiv.org/abs/2605.03604";
const DOI = "10.48550/arxiv.2605.03604";
const OPENALEX = "W7160408279";
const TITLE = "Multi-Agent Strategic Games with LLMs";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-STRATEGIC-GAMES-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`strategic-games-metric-row-${index}`],
    flags: [],
    narrative: `Strategic-games source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "strategic-games-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 8).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `strategic-games-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `strategic-games-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0710 multi-agent strategic games metric-validity boundary", () => {
  it("documents live arXiv metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0710");
    expect(doc).toContain(SOURCE);
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("arXiv `2605.03604`");
    expect(doc).toContain("5 May 2026");
    expect(doc).toContain("Maxim Chupilkin");
    expect(doc).toContain("Computer Science and Game Theory");
    expect(doc).toContain("Computers and Society");
    expect(doc).toContain("29 pages, 6 figures");
    expect(doc).toContain("security-dilemma experiments");
    expect(doc).toContain("multipolarity");
    expect(doc).toContain("finite horizons");
    expect(doc).toContain("communication");
    expect(doc).toContain("public messages");
    expect(doc).toContain("private reasoning");
  ...
  it("accepts strategic-game context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "strategic-games-construct-validity",
      "strategic-games-validation-table",
      "strategic-games-sample-size",
      "strategic-games-confidence-interval",
      "strategic-games-reliability-check",
      "strategic-games-regression-threshold",
      "strategic-games-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "strategic-games-context-eval-agent",
        runId: "run-gap0710-strategic-games-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        validationFacetChecks: [
          ...
          facetId: `strategic-games-${facetId}`,
          covered: true,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `strategic-games-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "strategic-games-context-score-predicts-coordination-quality",
          aligned: true,
          evidenceRefs: ["strategic-games-reliability-check", "strategic-games-regression-threshold"],
        }],
        sourceRefs: [SOURCE, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0710-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0710-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when arXiv metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "strategic-games-context-eval-agent",
      runId: "run-gap0710-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-STRATEGIC-GAMES-EVAL-01", layerName }],
      sourceRefs: [SOURCE],
      gateMode: "ci",
    });
  ...
  it("does not add strategic-game identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("strategic_games_metric_validity");
