import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { buildMetricValidationReport } from "../src/score/metricValidity.js";
import type { DiagnosticReport, LayerName, QuestionScore } from "../src/types.js";

const DOC = "docs/source-reviews/GAP-0870-maseval-metric-validity.md";
const REPO = "maseval/MASEval";
const URL = "https://github.com/maseval/MASEval";
const DOCS = "https://maseval.readthedocs.io/";
const TITLE = "LLM-based Multi-Agent Evaluation & Benchmark Framework";

const implementationFiles = [
  "src/score/metricValidity.ts",
  "src/methodology/publicMethodology.ts",
  "src/diagnostic/methodologyVersioning.ts",
  "docs/SCORING_METHODOLOGY.md",
];

const layerName: LayerName = "Evaluation and Improvement";

function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-MASEVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`maseval-metric-row-${index}`],
    flags: [],
    narrative: `MASEval source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "maseval-context-agent",
    runId,
    ts,
    layerScores: [{ layerName, avgFinalLevel: layerValue, confidenceWeightedFinalLevel: layerValue }],
  } as DiagnosticReport;
}

function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 570).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `maseval-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `maseval-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0870 MASEval metric-validity boundary", () => {
  it("documents live GitHub and docs metadata with required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0870");
    expect(doc).toContain(REPO);
    expect(doc).toContain(URL);
    expect(doc).toContain(DOCS);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("live GitHub repository page");
    expect(doc).toContain("live Read the Docs page");
    expect(doc).toContain("README.md");
    expect(doc).toContain("LICENSE");
    expect(doc).toContain("MIT license");
    expect(doc).toContain("Star 35");
    expect(doc).toContain("Fork 10");
    expect(doc).toContain("Issues 7");
    expect(doc).toContain("Pull requests 3");
    expect(doc).toContain("78 Commits");
    expect(doc).toContain("v0.4.0 Latest Mar 28, 2026");
    expect(doc).toContain("Python 99.8%");
    expect(doc).toContain("Just 0.2%");
    expect(doc).toContain(".github");
    expect(doc).toContain("assets");
    expect(doc).toContain("docs");
    expect(doc).toContain("examples");
    expect(doc).toContain("maseval");
    expect(doc).toContain("scripts");
    expect(doc).toContain("tests");
    expect(doc).toContain("BENCHMARKS.md");
    expect(doc).toContain("CITATION.cff");
    expect(doc).toContain("pyproject.toml");
    expect(doc).toContain("agentic-ai");
    expect(doc).toContain("multi-agent-systems");
    expect(doc).toContain("large-language-models");
    expect(doc).toContain("pip install maseval");
    expect(doc).toContain("AutoGen");
    expect(doc).toContain("LangChain");
    expect(doc).toContain("GAIA");
    expect(doc).toContain("AgentBench");
    expect(doc).toContain("system-level benchmarking");
    expect(doc).toContain("task-specific configurations");
    expect(doc).toContain("framework agnostic");
    expect(doc).toContain("lifecycle hooks");
    expect(doc).toContain("tracing");
    expect(doc).toContain("logging");
    expect(doc).toContain("metrics collection");
    expect(doc).toContain("ConVerse");
    expect(doc).toContain("GAIA2");
    expect(doc).toContain("MACS");
    expect(doc).toContain("MMLU");
    expect(doc).toContain("MultiAgentBench");
    expect(doc).toContain("Tau2");
    expect(doc).toContain("validation table");
    expect(doc).toContain("confidence interval");
    expect(doc).toContain("sample size");
    expect(doc).toContain("metric owner");
    expect(doc).toContain("## Relevance decision");
    expect(doc).toContain("## AMC/8 surface check");
    expect(doc).toContain("## Product closure");
    expect(doc).toContain("## Fail-closed rule");
    expect(doc).toContain("## No-bloat boundary");
    expect(doc).toContain("## Verification");
  });

  it("accepts MASEval context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "maseval-system-level-benchmarking",
      "maseval-validation-table",
      "maseval-sample-size",
      "maseval-confidence-interval",
      "maseval-reliability-check",
      "maseval-regression-threshold",
      "maseval-metric-owner",
      "maseval-framework-agnostic-alignment",
      "maseval-trace-first-alignment",
    ];
    const signedEvidenceRefs = [
      ...questionScores.map((row, index) => signedRef(row.evidenceEventIds[0]!, index)),
      ...evidenceIds.map((evidenceId, index) => signedRef(evidenceId, index + 100)),
    ];

    const report = buildMetricValidationReport(
      {
        agentId: "maseval-context-agent",
        runId: "run-gap0870-maseval-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        trustLabel: "HIGH TRUST",
        integrityIndex: 1,
        evidenceCoverage: 1,
        correlationRatio: 0.88,
        unsupportedClaimCount: 0,
        layerScores: [{ layerName, avgFinalLevel: 3, confidenceWeightedFinalLevel: 3 }],
        questionScores,
        confidenceSummary: {
          lowConfidenceFindings: 0,
          highUncertaintyFindings: 0,
          downgradedFindings: 0,
          autoFixBlockedRecommendations: 0,
          averageEvidenceSufficiency: 1,
          averageJudgeAgreement: 0.85,
        },
        questions: questionScores.map((row) => ({ id: row.questionId, layerName })),
        signedEvidenceRefs,
        validationFacetChecks: [
          "system-level-benchmarking",
          "framework-agnostic-eval",
          "trace-first-evidence",
          "task-specific-configs",
          "lifecycle-hook-observability",
        ].map((facetId, index) => ({
          facetId: `maseval-${facetId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index]!],
        })),
        processEvidenceChecks: [
          ["validation-table", "maseval-validation-table"],
          ["sample-size", "maseval-sample-size"],
          ["confidence-interval", "maseval-confidence-interval"],
          ["reliability-check", "maseval-reliability-check"],
          ["regression-threshold", "maseval-regression-threshold"],
          ["metric-owner", "maseval-metric-owner"],
        ].map(([processEvidenceId, evidenceId]) => ({
          processEvidenceId: `maseval-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceId],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "maseval-score-predicts-system-level-evaluation-quality",
          aligned: true,
          evidenceRefs: ["maseval-framework-agnostic-alignment", "maseval-trace-first-alignment"],
        }],
        sourceRefs: [URL, DOCS],
        gateMode: "ci",
      },
      [
        prior("run-gap0870-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0870-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );

    expect(report.failClosed).toBe(false);
    expect(report.rows[0]).toMatchObject({
      metricId: "overall_maturity_score",
      sampleSize: 20,
      validationFacetCoverage: 1,
      processEvidenceCoverage: 1,
      outcomeAlignment: 1,
      status: "pass",
    });
    expect(report.rows[0]?.confidenceInterval.level).toBe(0.95);
    expect(report.rows[0]?.interRaterAgreement).toBe(0.85);
    expect(report.evalPack.replayable).toBe(true);
    expect(report.evalPack.sourceRefs).toEqual([URL, DOCS]);
    expect(report.evalPack.rows[0]?.rowHash).toMatch(/^[a-f0-9]{64}$/);
    expect(report.ciGate.passed).toBe(true);
  });

  it("fails closed when MASEval metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "maseval-context-agent",
      runId: "run-gap0870-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      trustLabel: "LOW TRUST",
      integrityIndex: 0.2,
      evidenceCoverage: 0.1,
      correlationRatio: 0.2,
      unsupportedClaimCount: 1,
      layerScores: [{ layerName, avgFinalLevel: 1, confidenceWeightedFinalLevel: 1 }],
      questionScores: [{ ...score(1, 1, 0.12), evidenceEventIds: [], flags: ["FLAG_UNSUPPORTED_CLAIM"] }],
      questions: [{ id: "AMC-MASEVAL-01", layerName }],
      sourceRefs: [URL, DOCS],
      gateMode: "ci",
    });

    expect(report.failClosed).toBe(true);
    expect(report.evalPack.replayable).toBe(false);
    expect(report.ciGate.failClosed).toBe(true);
    expect(report.rows[0]?.warnings.join(" ")).toContain("sample size");
    expect(report.rows[0]?.warnings.join(" ")).toContain("construct validity");
  });

  it("does not add MASEval identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(REPO);
      expect(source).not.toContain(URL);
      expect(source).not.toContain("maseval_metric_validity");
      expect(source).not.toContain(TITLE);
    }
  });
});
