...
const DOC = "docs/source-reviews/GAP-0779-medical-clinic-public-methodology-unavailable.md";
const DOI = "10.3390/electronics15020334";
const OPENALEX = "W7122760289";
const TITLE = "Designing an Architecture of a Multi-Agentic AI-Powered Virtual Assistant Using LLMs and RAG for a Medical Clinic";
...
describe("GAP-0779 medical clinic public-methodology unavailable-source boundary", () => {
  it("documents failed live retrieval and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0779");
    ...
    expect(doc).toContain("multi-agent virtual assistants");
    expect(doc).toContain("medical-clinic workflows");
    expect(doc).toContain("RAG");
    expect(doc).toContain("interpretability");
    expect(doc).toContain("HCI");
  ...
  it("keeps unavailable medical-clinic metadata out of public methodology semantics", () => {
    const doc = readFileSync(DOC, "utf8");
    ...
    expect(manifestText).not.toContain("medical_clinic_public_methodology");
  ...
  it("keeps source-specific identifiers out of public methodology implementation modules", () => {
    for (const path of implementationFiles) {
      ...
      expect(source).not.toContain("medical_clinic_public_methodology");
    }
  });
});
