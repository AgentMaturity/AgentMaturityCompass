...
const DOC = "docs/source-reviews/GAP-0703-remote-sensing-multi-agent-metric-validity-unavailable.md";
const DOI = "10.1080/20964471.2025.2600178";
const OPENALEX = "W7119479065";
const TITLE = "An LLM-based multi-agent system for remote sensing analysis";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-REMOTE-SENSING-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`remote-sensing-metric-row-${index}`],
    flags: [],
    narrative: `Remote-sensing source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "remote-sensing-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 6).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `remote-sensing-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `remote-sensing-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0703 remote sensing multi-agent metric-validity unavailable-source boundary", () => {
  it("documents unavailable primary-source metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0703");
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("exact-title, DOI, OpenAlex, Taylor & Francis publisher-domain");
    ...
    expect(doc).toContain("remote sensing");
    expect(doc).toContain("visualization");
    expect(doc).toContain("security token");
  ...
  it("accepts remote-sensing context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "remote-sensing-construct-validity",
      "remote-sensing-validation-table",
      "remote-sensing-sample-size",
      "remote-sensing-confidence-interval",
      "remote-sensing-reliability-check",
      "remote-sensing-regression-threshold",
      "remote-sensing-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "remote-sensing-context-eval-agent",
        runId: "run-gap0703-remote-sensing-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        validationFacetChecks: [
          ...
          facetId: `remote-sensing-${facetId}`,
          covered: true,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `remote-sensing-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "remote-sensing-context-score-predicts-operational-quality",
          aligned: true,
          evidenceRefs: ["remote-sensing-reliability-check", "remote-sensing-regression-threshold"],
        }],
        sourceRefs: [`doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0703-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0703-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when paper metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "remote-sensing-context-eval-agent",
      runId: "run-gap0703-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-REMOTE-SENSING-EVAL-01", layerName }],
      sourceRefs: [`doi:${DOI}`],
      gateMode: "ci",
    });
  ...
  it("does not add remote-sensing identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("remote_sensing_metric_validity");
      expect(source).not.toContain("Remote sensing analysis");
    }
  });
});
