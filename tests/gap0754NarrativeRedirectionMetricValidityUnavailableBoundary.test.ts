...
const DOC = "docs/source-reviews/GAP-0754-narrative-redirection-metric-validity-unavailable.md";
const DOI = "10.1145/3742413.3789218";
const OPENALEX = "W7133345812";
const TITLE = "Guiding, Not Railroading: Design and Evaluation of a Multi-Agent System for Narrative Redirection in Role-playing Games";
...
function score(index: number, finalLevel = 3, confidence = 0.86): QuestionScore {
  return {
    questionId: `AMC-NARRATIVE-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`narrative-redirection-metric-row-${index}`],
    flags: [],
    narrative: `Narrative redirection source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "narrative-redirection-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    ...
    writerSig: `narrative-redirection-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `narrative-redirection-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0754 narrative redirection metric-validity unavailable-source boundary", () => {
  it("documents unavailable ACM metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0754");
    ...
    expect(doc).toContain("metric validity and reliability checks");
    expect(doc).toContain("narrative");
    expect(doc).toContain("coherence");
    expect(doc).toContain("agency");
    expect(doc).toContain("human-computer interaction");
    expect(doc).toContain("narrative network");
    expect(doc).toContain("adventure");
    expect(doc).toContain("role-playing game");
  ...
  it("accepts narrative-redirection context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "narrative-construct-validity",
      "narrative-validation-table",
      "narrative-sample-size",
      "narrative-confidence-interval",
      "narrative-reliability-check",
      "narrative-regression-threshold",
      "narrative-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "narrative-redirection-context-eval-agent",
        runId: "run-gap0754-narrative-redirection-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        trustLabel: "HIGH TRUST",
        integrityIndex: 1,
        evidenceCoverage: 1,
        correlationRatio: 0.90,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `narrative-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "narrative-context-score-predicts-roleplay-agency-and-coherence",
          aligned: true,
          evidenceRefs: ["narrative-reliability-check", "narrative-regression-threshold"],
        }],
        sourceRefs: [`doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0754-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0754-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
    ...
    expect(report.evalPack.sourceRefs).toEqual([`doi:${DOI}`, `openalex:${OPENALEX}`]);
    expect(report.evalPack.rows[0]?.rowHash).toMatch(/^[a-f0-9]{64}$/);
    expect(report.ciGate.passed).toBe(true);
  });

  it("fails closed when unavailable source metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "narrative-redirection-context-eval-agent",
      runId: "run-gap0754-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-NARRATIVE-EVAL-01", layerName }],
      sourceRefs: [`doi:${DOI}`],
      gateMode: "ci",
    });
  ...
  it("does not add narrative-redirection identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("narrative_redirection_metric_validity");
