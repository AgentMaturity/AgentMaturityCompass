...
const DOC = "docs/source-reviews/GAP-0728-avo-metric-validity.md";
const SOURCE = "https://arxiv.org/abs/2603.24517";
const DOI = "10.48550/arxiv.2603.24517";
const OPENALEX = "W7140855608";
const TITLE = "AVO: Agentic Variation Operators for Autonomous Evolutionary Search";
...
function score(index: number, finalLevel = 3, confidence = 0.89): QuestionScore {
  return {
    questionId: `AMC-AVO-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`avo-metric-row-${index}`],
    flags: [],
    narrative: `AVO source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "avo-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 14).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `avo-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `avo-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0728 AVO metric-validity boundary", () => {
  it("documents live arXiv metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0728");
    expect(doc).toContain(SOURCE);
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("Terry Chen");
    expect(doc).toContain("Zhifan Ye");
    expect(doc).toContain("Bing Xu");
    expect(doc).toContain("Zihao Ye");
    expect(doc).toContain("Timmy Liu");
    expect(doc).toContain("Ali Hassani");
    expect(doc).toContain("Tianqi Chen");
    expect(doc).toContain("Humphrey Shi");
    expect(doc).toContain("2026-03-25");
    expect(doc).toContain("autonomous coding-agent loops");
    expect(doc).toContain("current-lineage context");
    expect(doc).toContain("execution feedback");
    expect(doc).toContain("attention-kernel optimization");
    expect(doc).toContain("NVIDIA B200");
    expect(doc).toContain("grouped-query attention");
  ...
  it("accepts AVO context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "avo-construct-validity",
      "avo-validation-table",
      "avo-sample-size",
      "avo-confidence-interval",
      "avo-reliability-check",
      "avo-regression-threshold",
      "avo-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "avo-context-eval-agent",
        runId: "run-gap0728-avo-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `avo-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "avo-context-score-predicts-autonomous-optimization-quality",
          aligned: true,
          evidenceRefs: ["avo-reliability-check", "avo-regression-threshold"],
        }],
        sourceRefs: [SOURCE, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0728-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0728-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when arXiv metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "avo-context-eval-agent",
      runId: "run-gap0728-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-AVO-EVAL-01", layerName }],
      sourceRefs: [SOURCE],
      gateMode: "ci",
    });
  ...
  it("does not add AVO identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("avo_metric_validity");
