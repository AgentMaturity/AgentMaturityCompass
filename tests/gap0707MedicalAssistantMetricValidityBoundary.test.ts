...
const DOC = "docs/source-reviews/GAP-0707-medical-assistant-metric-validity.md";
const SOURCE = "https://github.com/souvikmajumder26/Multi-Agent-Medical-Assistant";
const REPO = "souvikmajumder26/Multi-Agent-Medical-Assistant";
...
function score(index: number, finalLevel = 3, confidence = 0.89): QuestionScore {
  return {
    questionId: `AMC-MEDICAL-ASSISTANT-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`medical-assistant-metric-row-${index}`],
    flags: [],
    narrative: `Medical-assistant source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "medical-assistant-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 7).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `medical-assistant-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `medical-assistant-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0707 medical assistant metric-validity boundary", () => {
  it("documents live medical-assistant repository metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0707");
    expect(doc).toContain(SOURCE);
    expect(doc).toContain(REPO);
    expect(doc).toContain("repository id `946168154`");
    expect(doc).toContain("default branch `main`");
    expect(doc).toContain("size `256280`");
    expect(doc).toContain("not archived");
    expect(doc).toContain("2025-05-02T22:11:45Z");
    expect(doc).toContain("multi-agent medical assistant");
    expect(doc).toContain("medical imaging analysis");
    expect(doc).toContain("retrieval-augmented generation");
    expect(doc).toContain("human-in-the-loop validation");
    expect(doc).toContain("confidence-based routing");
    expect(doc).toContain("input/output guardrails");
  ...
  it("accepts medical-assistant context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "medical-assistant-construct-validity",
      "medical-assistant-validation-table",
      "medical-assistant-sample-size",
      "medical-assistant-confidence-interval",
      "medical-assistant-reliability-check",
      "medical-assistant-regression-threshold",
      "medical-assistant-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "medical-assistant-context-eval-agent",
        runId: "run-gap0707-medical-assistant-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        validationFacetChecks: [
          ...
          facetId: `medical-assistant-${facetId}`,
          covered: true,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `medical-assistant-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "medical-assistant-context-score-predicts-nonclinical-operational-quality",
          aligned: true,
          evidenceRefs: ["medical-assistant-reliability-check", "medical-assistant-regression-threshold"],
        }],
        sourceRefs: [SOURCE],
        gateMode: "ci",
      },
      [
        prior("run-gap0707-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0707-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
    ...
    expect(report.evalPack.sourceRefs).toEqual([SOURCE]);
    expect(report.evalPack.rows[0]?.rowHash).toMatch(/^[a-f0-9]{64}$/);
    expect(report.ciGate.passed).toBe(true);
  });

  it("fails closed when medical repository metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "medical-assistant-context-eval-agent",
      runId: "run-gap0707-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-MEDICAL-ASSISTANT-EVAL-01", layerName }],
      sourceRefs: [SOURCE],
      gateMode: "ci",
    });
  ...
  it("does not add medical-assistant identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(REPO);
      expect(source).not.toContain(SOURCE);
      expect(source).not.toContain("medical_assistant_metric_validity");
      expect(source).not.toContain("medical diagnosis");
    }
  });
});
