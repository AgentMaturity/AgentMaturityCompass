...
const DOC = "docs/source-reviews/GAP-0684-lexa-metric-validity-unavailable.md";
const DOI = "10.1007/s11280-026-01407-w";
const OPENALEX = "W7133355586";
const TITLE = "LEXA: Legal case retrieval via graph contrastive learning with contextualised LLM embeddings";
...
describe("GAP-0684 LEXA metric-validity unavailable-source boundary", () => {
  it("documents failed live retrieval and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0684");
    ...
    expect(doc).toContain("Springer article search");
  ...
  it("keeps unavailable legal-retrieval metadata out of public metric-validity semantics", () => {
    const doc = readFileSync(DOC, "utf8");
    ...
    expect(doc).toContain("No LEXA legal retrieval metric-validity adapter");
    ...
    expect(manifestText).not.toContain("lexa_metric_validity");
  });
  ...
  it("keeps source-specific identifiers out of metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      ...
      expect(source).not.toContain("lexa_metric_validity");
    }
  });
});
