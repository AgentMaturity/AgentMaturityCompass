...
const DOC = "docs/source-reviews/GAP-0807-vesta-metric-validity.md";
const ARXIV = "https://arxiv.org/abs/2606.08531";
const DOI = "10.48550/arxiv.2606.08531";
const OPENALEX = "W7164034352";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-VESTA-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`vesta-metric-row-${index}`],
    flags: [],
    narrative: `VESTA source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "vesta-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 89).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `vesta-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `vesta-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0807 VESTA metric-validity boundary", () => {
  it("documents live arXiv metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0807");
    expect(doc).toContain(ARXIV);
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("same live source reviewed for GAP-0806");
  ...
  it("accepts VESTA safety context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "vesta-construct-validity",
      "vesta-validation-table",
      "vesta-sample-size",
      "vesta-confidence-interval",
      "vesta-reliability-check",
      "vesta-regression-threshold",
      "vesta-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "vesta-context-eval-agent",
        runId: "run-gap0807-vesta-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `vesta-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "vesta-score-predicts-safety-eval-quality",
          aligned: true,
          evidenceRefs: ["vesta-reliability-check", "vesta-regression-threshold"],
        }],
        sourceRefs: [ARXIV, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0807-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0807-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when source metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "vesta-context-eval-agent",
      runId: "run-gap0807-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-VESTA-EVAL-01", layerName }],
      sourceRefs: [ARXIV],
      gateMode: "ci",
    });
  ...
  it("does not add VESTA identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(ARXIV);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("vesta_metric_validity");
