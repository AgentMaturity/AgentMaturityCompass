...
const DOC = "docs/source-reviews/GAP-0720-data-prompt-co-evolution-metric-validity-unavailable.md";
const DOI = "10.1145/3772318.3791222";
const OPENALEX = "W7128480797";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-DATA-PROMPT-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`data-prompt-metric-row-${index}`],
    flags: [],
    narrative: `Data-prompt co-evolution source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "data-prompt-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 10).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `data-prompt-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `data-prompt-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0720 Data-Prompt Co-Evolution metric-validity unavailable-source boundary", () => {
  it("documents unavailable primary-source metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0720");
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("exact-title, DOI, OpenAlex, and ACM publisher-domain searches");
    ...
    expect(doc).toContain("data-prompt co-evolution");
    expect(doc).toContain("growing test sets");
    expect(doc).toContain("prompt refinement");
  ...
  it("accepts data-prompt co-evolution context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "data-prompt-construct-validity",
      "data-prompt-validation-table",
      "data-prompt-sample-size",
      "data-prompt-confidence-interval",
      "data-prompt-reliability-check",
      "data-prompt-regression-threshold",
      "data-prompt-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "data-prompt-context-eval-agent",
        runId: "run-gap0720-data-prompt-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `data-prompt-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "data-prompt-context-score-predicts-observed-evaluation-quality",
          aligned: true,
          evidenceRefs: ["data-prompt-reliability-check", "data-prompt-regression-threshold"],
        }],
        sourceRefs: [`doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0720-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0720-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when paper metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "data-prompt-context-eval-agent",
      runId: "run-gap0720-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-DATA-PROMPT-EVAL-01", layerName }],
      sourceRefs: [`doi:${DOI}`],
      gateMode: "ci",
    });
  ...
  it("does not add data-prompt identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("data_prompt_co_evolution_metric_validity");
