...
const DOC = "docs/source-reviews/GAP-0785-schema-retrieval-metric-validity.md";
const SOURCE = "https://www.mdpi.com/2076-3417/16/2/586";
const DOI = "10.3390/app16020586";
const DOI_URL = `https://doi.org/${DOI}`;
const OPENALEX = "W7118421099";
const TITLE = "Schema Retrieval with Embeddings and Vector Stores Using Retrieval-Augmented Generation and LLM-Based SQL Query Generation";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-SCHEMA-RETRIEVAL-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`schema-retrieval-metric-row-${index}`],
    flags: [],
    narrative: `Schema retrieval source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "schema-retrieval-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 31).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `schema-retrieval-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `schema-retrieval-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0785 schema retrieval metric-validity boundary", () => {
  it("documents live MDPI metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0785");
    expect(doc).toContain(SOURCE);
    expect(doc).toContain(DOI);
    expect(doc).toContain(DOI_URL);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("6 January 2026");
    expect(doc).toContain("Applied Sciences");
    expect(doc).toContain("16(2), 586");
    expect(doc).toContain("Mehmet Bozdemir");
    expect(doc).toContain("Metin Bilgin");
    expect(doc).toContain("AI-Based Data Science and Database Systems");
    expect(doc).toContain("hierarchical clustering");
    expect(doc).toContain("iterative repair mechanism");
    expect(doc).toContain("hybrid query strategy");
    expect(doc).toContain("Turkish + English");
    expect(doc).toContain("15 databases");
    expect(doc).toContain("1006 queries");
    expect(doc).toContain("schema retrieval F1 score from 0.79 to 0.88");
    expect(doc).toContain("GPT-4o");
    expect(doc).toContain("execution accuracy");
    expect(doc).toContain("0.70 to 0.78");
    expect(doc).toContain("11% improvement");
  ...
  it("accepts schema retrieval context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "schema-retrieval-construct-validity",
      "schema-retrieval-validation-table",
      "schema-retrieval-sample-size",
      "schema-retrieval-confidence-interval",
      "schema-retrieval-reliability-check",
      "schema-retrieval-regression-threshold",
      "schema-retrieval-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "schema-retrieval-context-eval-agent",
        runId: "run-gap0785-schema-retrieval-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        validationFacetChecks: [
          ...
          facetId: `schema-retrieval-${facetId}`,
          covered: true,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `schema-retrieval-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "schema-retrieval-score-predicts-text-to-sql-eval-quality",
          aligned: true,
          evidenceRefs: ["schema-retrieval-reliability-check", "schema-retrieval-regression-threshold"],
        }],
        sourceRefs: [SOURCE, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0785-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0785-repeat", Date.UTC(2026, 5, 14), 3.02),
      ],
    );
  ...
  it("fails closed when MDPI metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "schema-retrieval-context-eval-agent",
      runId: "run-gap0785-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-SCHEMA-RETRIEVAL-EVAL-01", layerName }],
      sourceRefs: [SOURCE],
      gateMode: "ci",
    });
  ...
  it("does not add schema retrieval identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("schema_retrieval_metric_validity");
