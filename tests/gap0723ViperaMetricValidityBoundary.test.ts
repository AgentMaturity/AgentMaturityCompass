...
const DOC = "docs/source-reviews/GAP-0723-vipera-metric-validity.md";
const SOURCE = "https://arxiv.org/abs/2510.05742";
const DOI = "10.1145/3772318.3791942";
const OPENALEX = "W4414978759";
const TITLE = "Vipera: Blending Visual and LLM-Driven Guidance for Systematic Auditing of Text-to-Image Generative AI";
...
function score(index: number, finalLevel = 3, confidence = 0.89): QuestionScore {
  return {
    questionId: `AMC-VIPERA-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`vipera-metric-row-${index}`],
    flags: [],
    narrative: `Vipera source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "vipera-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 11).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `vipera-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `vipera-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0723 Vipera metric-validity boundary", () => {
  it("documents live arXiv metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0723");
    expect(doc).toContain(SOURCE);
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("Yanwei Huang");
    expect(doc).toContain("Wesley Hanwen Deng");
    expect(doc).toContain("Sijia Xiao");
    expect(doc).toContain("Motahhare Eslami");
    expect(doc).toContain("Jason I. Hong");
    expect(doc).toContain("Arpit Narechania");
    expect(doc).toContain("Adam Perer");
    expect(doc).toContain("2025-10-07");
    expect(doc).toContain("five AI auditors");
    expect(doc).toContain("scene graphs");
    expect(doc).toContain("LLM-powered suggestions");
    expect(doc).toContain("24 participants");
  ...
  it("accepts Vipera context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "vipera-construct-validity",
      "vipera-validation-table",
      "vipera-sample-size",
      "vipera-confidence-interval",
      "vipera-reliability-check",
      "vipera-regression-threshold",
      "vipera-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "vipera-context-eval-agent",
        runId: "run-gap0723-vipera-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `vipera-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "vipera-context-score-predicts-systematic-audit-quality",
          aligned: true,
          evidenceRefs: ["vipera-reliability-check", "vipera-regression-threshold"],
        }],
        sourceRefs: [SOURCE, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0723-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0723-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when arXiv metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "vipera-context-eval-agent",
      runId: "run-gap0723-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-VIPERA-EVAL-01", layerName }],
      sourceRefs: [SOURCE],
      gateMode: "ci",
    });
  ...
  it("does not add Vipera identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("vipera_metric_validity");
