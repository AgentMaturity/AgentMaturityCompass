...
const DOC = "docs/source-reviews/GAP-0752-vistoryteller-metric-validity-unavailable.md";
const DOI = "10.1145/3742413.3789086";
const OPENALEX = "W7133303600";
const TITLE = "Vistoryteller: Designing Data Stories with LLM Agent-Based Generation and Interactive User Control";
...
function score(index: number, finalLevel = 3, confidence = 0.87): QuestionScore {
  return {
    questionId: `AMC-VISTORYTELLER-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`vistoryteller-metric-row-${index}`],
    flags: [],
    narrative: `Vistoryteller source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "vistoryteller-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    ...
    writerSig: `vistoryteller-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `vistoryteller-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0752 Vistoryteller metric-validity unavailable-source boundary", () => {
  it("documents unavailable ACM metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0752");
    ...
    expect(doc).toContain("data stories");
    ...
    expect(doc).toContain("storytelling");
  ...
  it("accepts data-story context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "vistoryteller-construct-validity",
      "vistoryteller-validation-table",
      "vistoryteller-sample-size",
      "vistoryteller-confidence-interval",
      "vistoryteller-reliability-check",
      "vistoryteller-regression-threshold",
      "vistoryteller-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "vistoryteller-context-eval-agent",
        runId: "run-gap0752-vistoryteller-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `vistoryteller-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "vistoryteller-context-score-predicts-data-story-quality",
          aligned: true,
          evidenceRefs: ["vistoryteller-reliability-check", "vistoryteller-regression-threshold"],
        }],
        sourceRefs: [`doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0752-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0752-repeat", Date.UTC(2026, 5, 14), 3.02),
      ],
    );
  ...
  it("fails closed when unavailable source metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "vistoryteller-context-eval-agent",
      runId: "run-gap0752-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-VISTORYTELLER-EVAL-01", layerName }],
      sourceRefs: [`doi:${DOI}`],
      gateMode: "ci",
    });
  ...
  it("does not add Vistoryteller identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("vistoryteller_metric_validity");
