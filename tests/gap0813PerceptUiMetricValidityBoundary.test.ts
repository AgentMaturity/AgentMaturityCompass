...
const DOC = "docs/source-reviews/GAP-0813-perceptui-metric-validity.md";
const ARXIV = "https://arxiv.org/abs/2606.05697";
const DOI = "10.48550/arxiv.2606.05697";
const OPENALEX = "W7163715795";
...
function score(index: number, finalLevel = 3, confidence = 0.91): QuestionScore {
  return {
    questionId: `AMC-PERCEPTUI-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`perceptui-metric-row-${index}`],
    flags: [],
    narrative: `PerceptUI source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "perceptui-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 97).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `perceptui-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `perceptui-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0813 PerceptUI metric-validity boundary", () => {
  it("documents the PerceptUI DOI alias and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0813");
    expect(doc).toContain(ARXIV);
    expect(doc).toContain(DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("same live source reviewed for GAP-0812");
  ...
  it("accepts PerceptUI context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 18 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "perceptui-construct-validity",
      "perceptui-validation-table",
      "perceptui-sample-size",
      "perceptui-confidence-interval",
      "perceptui-reliability-check",
      "perceptui-regression-threshold",
      "perceptui-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "perceptui-context-eval-agent",
        runId: "run-gap0813-perceptui-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        trustLabel: "HIGH TRUST",
        integrityIndex: 1,
        evidenceCoverage: 1,
        correlationRatio: 0.89,
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `perceptui-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "perceptui-score-predicts-uiux-eval-quality",
          aligned: true,
          evidenceRefs: ["perceptui-reliability-check", "perceptui-regression-threshold"],
        }],
        sourceRefs: [ARXIV, `doi:${DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0813-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0813-repeat", Date.UTC(2026, 5, 14), 3.02),
      ],
    );
  ...
  it("fails closed when source metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "perceptui-context-eval-agent",
      runId: "run-gap0813-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-PERCEPTUI-EVAL-01", layerName }],
      sourceRefs: [ARXIV],
      gateMode: "ci",
    });
  ...
  it("does not add PerceptUI identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(DOI);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("perceptui_metric_validity");
