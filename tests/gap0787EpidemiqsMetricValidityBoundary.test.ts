...
const DOC = "docs/source-reviews/GAP-0787-epidemiqs-metric-validity.md";
const ARXIV = "https://arxiv.org/abs/2510.00024";
const ARXIV_DOI = "10.48550/arXiv.2510.00024";
const IEEE_DOI = "10.1109/tai.2026.3666830";
const OPENALEX = "W7131082530";
const TITLE = "EpidemIQs: Prompt-to-Paper LLM Agents for Epidemic Modeling and Analysis";
...
function score(index: number, finalLevel = 3, confidence = 0.9): QuestionScore {
  return {
    questionId: `AMC-EPIDEMIQS-EVAL-${index.toString().padStart(2, "0")}`,
    claimedLevel: finalLevel,
    supportedMaxLevel: finalLevel,
    finalLevel,
    confidence,
    evidenceEventIds: [`epidemiqs-metric-row-${index}`],
    flags: [],
    narrative: `EpidemIQs source-review context is bounded to AMC metric-validity sample ${index}`,
  };
}

function prior(runId: string, ts: number, layerValue = 3): DiagnosticReport {
  return {
    agentId: "epidemiqs-context-eval-agent",
    runId,
    ts,
...
function signedRef(evidenceId: string, index: number) {
  return {
    evidenceId,
    eventHash: `${(index + 43).toString(16)}`.repeat(64).slice(0, 64),
    writerSig: `epidemiqs-metric-writer-${index}`,
    eventType: "metric" as const,
    sessionId: `epidemiqs-metric-session-${index}`,
    ts: Date.UTC(2026, 5, 21),
    trustTier: "OBSERVED_HARDENED" as const,
  };
}

describe("GAP-0787 EpidemIQs metric-validity boundary", () => {
  it("documents live arXiv metadata and required source-review sections", () => {
    const doc = readFileSync(DOC, "utf8");

    expect(doc).toContain("GAP-0787");
    expect(doc).toContain(ARXIV);
    expect(doc).toContain(ARXIV_DOI);
    expect(doc).toContain(IEEE_DOI);
    expect(doc).toContain(OPENALEX);
    expect(doc).toContain(TITLE);
    expect(doc).toContain("submitted `24 Sep 2025`");
    expect(doc).toContain("last revised `25 Feb 2026`");
    expect(doc).toContain("v2");
    expect(doc).toContain("Mohammad Hossein Samaei");
    expect(doc).toContain("Faryad Darabi Sahneh");
    expect(doc).toContain("Lee W. Cohnstaedt");
    expect(doc).toContain("Caterina Scoglio");
    expect(doc).toContain("Social and Information Networks");
    expect(doc).toContain("Artificial Intelligence");
    expect(doc).toContain("five predefined research phases");
    expect(doc).toContain("scientist agent");
    expect(doc).toContain("task-expert agent");
    expect(doc).toContain("GPT 4.1");
    expect(doc).toContain("GPT 4.1 Mini");
    expect(doc).toContain("870K");
    expect(doc).toContain("$1.57 per study");
    expect(doc).toContain("average task success rate of 79%");
    expect(doc).toContain("LLM-as-Judge and human expert reviews");
  ...
  it("accepts epidemic-modeling agent context only through existing metric-validity receipts", () => {
    const questionScores = Array.from({ length: 20 }, (_, index) => score(index + 1));
    const evidenceIds = [
      "epidemiqs-construct-validity",
      "epidemiqs-validation-table",
      "epidemiqs-sample-size",
      "epidemiqs-confidence-interval",
      "epidemiqs-reliability-check",
      "epidemiqs-regression-threshold",
      "epidemiqs-metric-owner",
    ];
    ...
    const report = buildMetricValidationReport(
      {
        agentId: "epidemiqs-context-eval-agent",
        runId: "run-gap0787-epidemiqs-metric-validity",
        ts: Date.UTC(2026, 5, 21),
        ...
        processEvidenceChecks: [
          ...
          processEvidenceId: `epidemiqs-${processEvidenceId}`,
          covered: true,
          evidenceRefs: [evidenceIds[index + 1]!],
        })),
        outcomeAlignmentChecks: [{
          outcomeId: "epidemiqs-score-predicts-epidemic-agent-eval-quality",
          aligned: true,
          evidenceRefs: ["epidemiqs-reliability-check", "epidemiqs-regression-threshold"],
        }],
        sourceRefs: [ARXIV, `doi:${ARXIV_DOI}`, `doi:${IEEE_DOI}`, `openalex:${OPENALEX}`],
        gateMode: "ci",
      },
      [
        prior("run-gap0787-baseline", Date.UTC(2026, 5, 7), 3),
        prior("run-gap0787-repeat", Date.UTC(2026, 5, 14), 3.01),
      ],
    );
  ...
  it("fails closed when paper metadata replaces signed metric-validity evidence", () => {
    const report = buildMetricValidationReport({
      agentId: "epidemiqs-context-eval-agent",
      runId: "run-gap0787-metadata-only",
      ts: Date.UTC(2026, 5, 21),
      ...
      questions: [{ id: "AMC-EPIDEMIQS-EVAL-01", layerName }],
      sourceRefs: [ARXIV],
      gateMode: "ci",
    });
  ...
  it("does not add EpidemIQs identifiers to metric-validity implementation modules", () => {
    for (const path of implementationFiles) {
      const source = readFileSync(path, "utf8");
      expect(source).not.toContain(ARXIV);
      expect(source).not.toContain(OPENALEX);
      expect(source).not.toContain("epidemiqs_metric_validity");
