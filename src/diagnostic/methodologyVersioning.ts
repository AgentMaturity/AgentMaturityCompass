...
export const CUA_COMPUTER_USE_SOURCE_REVIEW_REF = "github:trycua/cua; readme:https://raw.githubusercontent.com/trycua/cua/main/README.md; bench-readme:https://raw.githubusercontent.com/trycua/cua/main/libs/cua-bench/README.md; docs:https://cua.ai/docs/cuabench/guide/getting-started/introduction";
...
const CUA_COMPUTER_USE_REQUIRED_AUDIT_FIELDS = [
...
export function buildDiagnosticMethodologyVersioningReceipt(
  ...
  const cuaComputerUseBoundary = manifest.scoreClaimBoundaries.find((boundary) => boundary.boundary === "cua_computer_use_public_methodology");
  const cuaComputerUseGate = manifest.metricValidationGates.find((gate) => gate.gate === "cua_computer_use_public_methodology");
  ...
  const checks = [
    ...
    { field: "sourceReview.lunary.methodologyVersioning", present: lunaryBoundary?.requiredEvidence.includes("methodology version") === true && lunaryBoundary?.requiredEvidence.includes("changelog") === true && lunaryBoundary?.requiredEvidence.includes("deprecation notice") === true && lunaryBoundary?.requiredEvidence.includes("migration guidance") === true && lunaryBoundary?.requiredEvidence.includes("badge-assurance hash") === true },
    ...
    { field: "sourceReview.lunary.metricGate", present: lunaryGate?.defaultThreshold.includes("validationTable present") === true && lunaryGate?.defaultThreshold.includes("methodologyVersion present") === true && lunaryGate?.defaultThreshold.includes("metadata-only") === true },
    { field: "sourceReview.cuaComputerUse.boundary", present: cuaComputerUseBoundary?.appliesWhen.includes("Score report") === true && cuaComputerUseBoundary?.appliesWhen.includes("Shield receipt") === true && cuaComputerUseBoundary?.appliesWhen.includes("Watch alert") === true },
    { field: "sourceReview.cuaComputerUse.liveGithubMetadata", present: cuaComputerUseBoundary?.requiredEvidence.includes("Live GitHub metadata relevance review") === true && cuaComputerUseBoundary?.requiredEvidence.includes("repository snapshot hash") === true },
    { field: "sourceReview.cuaComputerUse.benchmarkTaskProof", present: cuaComputerUseBoundary?.requiredEvidence.includes("task dataset manifest") === true && cuaComputerUseBoundary?.requiredEvidence.includes("environment image manifest") === true && cuaComputerUseBoundary?.requiredEvidence.includes("setup script and test script hashes") === true && cuaComputerUseBoundary?.requiredEvidence.includes("oracle solution proof") === true },
    { field: "sourceReview.cuaComputerUse.methodologyVersioning", present: cuaComputerUseBoundary?.requiredEvidence.includes("methodology version") === true && cuaComputerUseBoundary?.requiredEvidence.includes("changelog") === true && cuaComputerUseBoundary?.requiredEvidence.includes("deprecation notice") === true && cuaComputerUseBoundary?.requiredEvidence.includes("migration guidance") === true && cuaComputerUseBoundary?.requiredEvidence.includes("badge-assurance hash") === true },
    { field: "sourceReview.cuaComputerUse.metadataOnlyFailClosed", present: cuaComputerUseBoundary?.publicDisclosure.includes("source metadata alone") === true && cuaComputerUseBoundary?.publicDisclosure.includes("not a Cua integration") === true && cuaComputerUseBoundary?.publicDisclosure.includes("not a parity claim") === true },
    { field: "sourceReview.cuaComputerUse.noCopyBoundary", present: cuaComputerUseBoundary?.publicDisclosure.includes("does not authorize copied Cua README prose") === true },
    { field: "sourceReview.cuaComputerUse.metricGate", present: cuaComputerUseGate?.defaultThreshold.includes("taskDatasetManifest present") === true && cuaComputerUseGate?.defaultThreshold.includes("methodologyVersion present") === true && cuaComputerUseGate?.defaultThreshold.includes("metadata-only GitHub source review fails closed") === true },
  ...
  const receiptWithoutHash = {
    ...
    warnings: [
      ...
      "Lunary public product metadata is a relevant source-review signal for observability/evaluation boundaries, but metadata, labels, screenshots, copied website prose, examples, response scores, or dashboard exports fail closed without AMC-owned eval packs, validation tables, trace/session exports, evaluator configs, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owners, sample sizes, confidence intervals, thresholds, signed evidence, and row hashes.",
      "trycua/cua public repository, README, and Cua-Bench docs metadata are relevant source-review signals for computer-use benchmark public-methodology boundaries, but repository labels, star/fork/issue counts, OS or sandbox labels, benchmark command names, OSWorld/ScreenSpot/Windows Arena labels, trajectory filenames, aggregate scores, copied README/docs/code/config/task/trajectory content, or source metadata fail closed without AMC-owned eval packs, task manifests, environment manifests, setup/test/oracle proof, validation tables, thresholds, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owners, sample sizes, confidence intervals, signed evidence, no-copy proof, and row hashes.",
