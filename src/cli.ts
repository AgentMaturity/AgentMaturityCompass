...
program
  ...
  .action(async (opts: { dryRun: boolean; json: boolean }) => {
    const { runDoctorFix, renderDoctorFixReport } = await import("./doctor/doctorFix.js");
  ...
  .action(async (opts: { json: boolean; quiet: boolean; euAiAct: boolean; auto: boolean; rapid: boolean; agent?: string; share: boolean }) => {
    ...
    if (opts.rapid) {
      ...
      if (opts.share) {
        ...
        console.log(`[![AMC L${levelNum}](${shieldsUrl})](https://github.com/AgentMaturity/AgentMaturityCompass)`);
        console.log("");
        console.log(chalk.gray("HTML:"));
        console.log(`<a href="https://github.com/AgentMaturity/AgentMaturityCompass"><img src="${shieldsUrl}" alt="AMC L${levelNum}" /></a>`);
        console.log("");
...
lifecycle
  ...
  .action(async (opts: {
    ...
    const { lifecycleAdvanceCli, parseControlsCsv } = await import("./lifecycle/lifecycleCli.js");
    try {
...
program
  ...
  .action(async (opts: { agent?: string }) => {
    ...
    } = await import("./claims/governanceLineage.js");
    const ledger = openLedger(process.cwd());
    const db = ledger.db;
    initGovernanceLineageTables(db);
    const agentId = opts.agent ?? activeAgent(program) ?? "default";
...
program
  ...
  .action(async (opts: { agent?: string }) => {
    ...
    } = await import("./claims/claimConfidence.js");
    const ledger = openLedger(process.cwd());
    const db = ledger.db;
    const agentId = opts.agent ?? activeAgent(program) ?? "default";
...
mechanic
  ...
  .option("--gap-file <path>", "specific mechanic gap report JSON (defaults to latest signed report)")
  .option("--format <format>", "reward|dspy|recipe|all", "all")
  .option("--out <path>", "write exported artifact to file")
  .option("--json", "Output as JSON")
  .action((opts: { gapFile?: string; format?: string; out?: string; json?: boolean }) => {
    const format = opts.format === "reward" || opts.format === "dspy" || opts.format === "recipe" || opts.format === "all"
      ? opts.format
      : "all";
    const out = mechanicTuneExportCli({
      workspace: process.cwd(),
      gapFile: opts.gapFile,
      format,
      outFile: opts.out,
    });
    if (opts.json) {
    ...
    console.log(chalk.bold.hex('#4AEF79')("\n🛠️  Mechanic Export"));
    console.log(chalk.gray("Source gap:"), out.sourcePath);
    console.log(chalk.gray("Format:"), out.format);
    if (format === "reward") {
      const reward = out.output as { totalGaps: number; rewardSignals: unknown[] };
      console.log(chalk.gray("Reward signals:"), reward.rewardSignals.length);
      console.log(chalk.gray("Total gaps:"), reward.totalGaps);
    } else if (format === "dspy") {
      const dspy = out.output as { metrics: unknown[]; constraints: unknown[] };
      console.log(chalk.gray("Metrics:"), dspy.metrics.length);
      console.log(chalk.gray("Constraints:"), dspy.constraints.length);
    } else if (format === "recipe") {
      const recipe = out.output as { focusAreas: unknown[]; estimatedTrainingHours: number };
      console.log(chalk.gray("Focus areas:"), recipe.focusAreas.length);
      console.log(chalk.gray("Estimated training hours:"), recipe.estimatedTrainingHours);
    } else {
      const bundle = out.output as {
        reward: { rewardSignals: unknown[] };
        dspy: { metrics: unknown[] };
        recipe: { focusAreas: unknown[]; estimatedTrainingHours: number };
      };
      console.log(chalk.gray("Reward signals:"), bundle.reward.rewardSignals.length);
      console.log(chalk.gray("DSPy metrics:"), bundle.dspy.metrics.length);
      console.log(chalk.gray("Focus areas:"), bundle.recipe.focusAreas.length);
      console.log(chalk.gray("Estimated training hours:"), bundle.recipe.estimatedTrainingHours);
    }
    if (opts.out) {
      console.log(chalk.green(`Export written: ${resolve(process.cwd(), opts.out)}`));
    }
  });
...
score
  .command("a2a-protocol")
  .description("Score agent-to-agent protocol maturity: card completeness, lifecycle, auth, format, errors, discovery")
  .option("--file <path>", "JSON file containing { events: A2AProtocolEvent[] }")
  .option("--json", "Output as JSON")
  .action(async (opts: { file?: string; json?: boolean }) => {
    try {
      const { scoreA2AProtocol } = await import("./score/a2aProtocol.js");
      ...
      console.log(chalk.gray("Inter-agent auth:"), result.interAgentAuth.toFixed(2));
      console.log(chalk.gray("Message conformance:"), result.messageFormatConformance.toFixed(2));
      console.log(chalk.gray("Error handling:"), result.errorHandling.toFixed(2));
      console.log(chalk.gray("Discovery/routing:"), result.discoveryAndRouting.toFixed(2));
      if (result.maturitySignals.length > 0) {
        console.log(chalk.green("\n✅ Maturity signals:"));
        for (const s of result.maturitySignals) console.log(chalk.green(`  - ${s}`));
      }
      if (result.recommendations.length > 0) {
        console.log(chalk.yellow("\n💡 Recommendations:"));
        for (const r of result.recommendations) console.log(chalk.yellow(`  - ${r}`));
      ...
      if (!opts.file) {
        console.log(chalk.gray("\nTip: pass --file events.json to score recorded A2A traces."));
      }
    } catch (e: unknown) { console.error(chalk.red(toErrorMessage(e))); process.exit(1); }
  });

score
  .command("distributed-agents")
  .description("Score distributed multi-agent execution: partitions, sync, failover, consensus, load, observability")
  .option("--file <path>", "JSON file containing { events: DistributedAgentEvent[] }")
  .option("--json", "Output as JSON")
  .action(async (opts: { file?: string; json?: boolean }) => {
    try {
      const { scoreDistributedAgents } = await import("./score/distributedAgents.js");
      const input = opts.file
        ? JSON.parse(readUtf8(resolve(process.cwd(), opts.file))) as { events: unknown[] }
        : { events: [] };
      ...
      console.log(chalk.gray("Failover recovery:"), result.failoverRecovery.toFixed(2));
      console.log(chalk.gray("Consensus behavior:"), result.consensusBehavior.toFixed(2));
      console.log(chalk.gray("Load distribution:"), result.loadDistribution.toFixed(2));
      console.log(chalk.gray("Observability depth:"), result.observabilityDepth.toFixed(2));
      if (result.maturitySignals.length > 0) {
        console.log(chalk.green("\n✅ Maturity signals:"));
        for (const s of result.maturitySignals) console.log(chalk.green(`  - ${s}`));
      }
      ...
      if (!opts.file) {
        console.log(chalk.gray("\nTip: pass --file events.json to score recorded distributed-system traces."));
      }
    } catch (e: unknown) { console.error(chalk.red(toErrorMessage(e))); process.exit(1); }
  });
...
score
  ...
  .description("Score deep memory infrastructure: backend resilience, compression fidelity, cross-session consistency, TTL, capacity")
  .option("--file <path>", "JSON file containing { events: MemoryDepthEvent[] }")
  .option("--json", "Output as JSON")
  .action(async (opts: { file?: string; json?: boolean }) => {
    try {
      const { scoreMemoryDepth } = await import("./score/memoryDepth.js");
      const input = opts.file
        ? JSON.parse(readUtf8(resolve(process.cwd(), opts.file))) as { events: unknown[] }
        : { events: [] };
      ...
      console.log(chalk.gray("Cross-session consistency:"), result.crossSessionConsistency.toFixed(2));
      console.log(chalk.gray("Poisoning resistance:"), result.poisoningResistance.toFixed(2));
      console.log(chalk.gray("TTL compliance:"), result.ttlPolicyCompliance.toFixed(2));
      console.log(chalk.gray("Capacity management:"), result.capacityManagement.toFixed(2));
      if (result.maturitySignals.length > 0) {
        console.log(chalk.green("\n✅ Maturity signals:"));
        for (const s of result.maturitySignals) console.log(chalk.green(`  - ${s}`));
      }
      ...
      if (!opts.file) {
        console.log(chalk.gray("\nTip: pass --file events.json to score recorded memory-system events."));
      }
    } catch (e: unknown) { console.error(chalk.red(toErrorMessage(e))); process.exit(1); }
  });
        ...
        runs: [{
          tool: {
            driver: {
              ...
              version: "1.0.0",
              informationUri: "https://github.com/AgentMaturity/AgentMaturityCompass",
            }
          },
