...
export function getPublicMethodologyManifest(questionSet?: DiagnosticQuestionSetInfo): PublicMethodologyManifest {
  const manifestWithoutHash = {
    ...
    evaluationModeTaxonomy: [
      ...
      },
      {
        mode: "provider_drift",
        surface: "Watch, API",
        publicMeaning: "Provider/model canary comparisons across versions, capability slices, evaluator-library proof, Galileo eval-observability proof, observability-pipeline proof, geospatial tool-calling proof, AgentDefense-Bench MCP security-defense proof, Evidra evidence-chain proof, and evaluator reliability signals.",
        proofBinding: "provider drift benchmark eval-pack rows, evaluator-framework proof fields, Galileo source-signal/project/dataset/trace/metric/canary/drift/alert/signed-evidence proof fields, observability pipeline provenance fields, geospatial task/dataset/tool/trace/judge proof fields, AgentDefense-Bench source/MCP/attack/defense/run/drift/alert proof fields, Evidra evidence-chain proof fields, and Watch alert projections",
        limitation: "Provider drift receipts should disclose canary family, evaluator framework/version, Galileo source signal when used, observability pipeline, geospatial benchmark identity when used, AgentDefense-Bench MCP security-defense identity when used, Evidra evidence-chain identity when used, metric suite, sample size, signed evidence, and waiver state before procurement or compliance use."
      },
      {
    ...
    benchmarkMethodologyVersioning: {
      ...
      changeTriggers: [
        ...
        {
          trigger: "arize_phoenix_eval_observability_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when Arize Phoenix/AX-style observability or evaluation metric-validity semantics change source-docs verification, trace/span export, evaluator task/config, dataset or experiment manifest, annotation/eval result, monitoring alert, threshold, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing Arize Phoenix/AX-style tracing, online/offline evals, datasets, experiments, evaluator task, annotation, monitoring, alert, or metric-validity claims. Public docs and product-page metadata can seed review only; they are not parity proof."
        },
        {
          trigger: "lunary_observability_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, badge, and methodology versions when Lunary-style observability/evaluation semantics change live source receipt, trace/session export, prompt or model registry, score/evaluator config, feedback export, monitoring/alert policy, validation table, threshold, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing Lunary-style LLM observability, prompt templates, agent tracing, response scoring, analytics, feedback tracking, monitoring alerts, public-methodology, or metric-validity claims. Product-page metadata can seed review only; it is not parity proof."
        },
        {
          trigger: "google_adk_eval_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when Google ADK-style agent development/evaluation source-review semantics change live GitHub metadata review, validation table, evaluator-suite proof, trace-evaluation proof, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing Google ADK-style agent-evaluation or deployment claims. GitHub repository metadata can seed review only; it is not parity proof and must not create an ADK subsystem, adapter, importer, or copied upstream implementation."
        },
        {
          trigger: "lm_evaluation_harness_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when LM Evaluation Harness-style metric-validity source-review semantics change live GitHub metadata review, validation table, evaluator-suite proof, trace-evaluation proof, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing EleutherAI/lm-evaluation-harness-style evaluator, task, model benchmark, leaderboard, harness-run, or metric-validity claims. GitHub repository metadata can seed review only; it is not parity proof and must not create an lm-evaluation-harness subsystem, SDK/importer, adapter, parity layer, or copied upstream implementation."
        },
        {
          trigger: "openai_evals_public_methodology_change",
          versionImpact: "Increment methodology, scoring, verification-protocol, reporting, and badge versions when OpenAI Evals-style public-methodology semantics change live GitHub metadata review, eval-pack or dataset/case manifests, evaluator-suite proof through existing primitives, threshold policy, changelog, deprecation notice, migration guidance, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing openai/evals-style evaluation-framework, registry, benchmark, dataset, eval-run, or public-methodology claims. GitHub repository metadata can seed review only; it is not parity proof and must not create an OpenAI Evals subsystem, importer, adapter, registry mirror, or copied upstream implementation."
        },
        {
          trigger: "pocketflow_public_methodology_change",
          versionImpact: "Increment methodology, scoring, verification-protocol, reporting, and badge versions when PocketFlow-style public-methodology semantics change live GitHub metadata review, docs/module source-review scope, eval-pack or flow/case manifests when claimed, evaluator-suite proof through existing primitives, threshold policy, changelog, deprecation notice, migration guidance, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing The-Pocket/PocketFlow-style lightweight agent workflow framework, docs, flow/node, agent, workflow, or public-methodology claims. GitHub repository metadata can seed review only; it is not parity proof and must not create a PocketFlow subsystem, SDK/importer, adapter, parity layer, or copied upstream implementation."
        },
        {
          trigger: "openai_simple_evals_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when OpenAI Simple Evals-style metric-validity source-review semantics change live GitHub metadata review, validation table, evaluator-suite proof, trace-evaluation proof, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing openai/simple-evals-style lightweight evaluator, task, benchmark, local eval-run, result, or metric-validity claims. GitHub repository metadata can seed review only; it is not parity proof and must not create an OpenAI Simple Evals subsystem, SDK/importer, adapter, parity layer, or copied upstream implementation."

        },
        {
          trigger: "digital_materials_ecosystem_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when Digital materials ecosystem-style source-review semantics change DOI/OpenAlex verification, validation table, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing Digital materials ecosystem-cited agent-evaluation, autonomous-discovery, database/tool, or metric-validity claims. DOI/OpenAlex metadata can seed review only; it is not parity proof."
        },
        {
          trigger: "chemgraph_agentic_chemistry_workflow_metric_validity_change",
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when ChemGraph-style agentic computational chemistry workflow source-review semantics change DOI/OpenAlex verification, validation table, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing ChemGraph-cited agentic computational chemistry workflow, task-planning, multi-agent, simulation-workflow, benchmark, or metric-validity claims. DOI/OpenAlex metadata can seed review only; it is not parity proof and must not create a chemistry/domain subsystem, connector, importer, parity layer, or copied paper data/prose."
        },
        {
      ...
      },
      {
        ...
        {
          trigger: "fact_checking_factuality_review_methodology_change",
          versionImpact: "Increment methodology, scoring, badge-assurance, and source-review versions when fact-checking/factuality-evaluation review semantics change for DOI/OpenAlex metadata, claim decomposition, evidence retrieval, verification labels, factuality metric taxonomy, evaluator calibration, abstention/uncertainty policy, source freshness, or threshold and migration guidance.",
          migration: "Regenerate badges or reports before comparing review-informed fact-checking, factuality evaluation, hallucination detection, claim verification, source grounding, evidence retrieval, abstention, or evaluator-calibration methodology claims."
        },
        {
    ...
    scoreClaimBoundaries: [
      {
        boundary: "artifact_evidence_claim_readiness",
        appliesWhen: "Any diagnostic report, lifecycle artifact, Studio view, share bundle, executive brief, badge, or public claim relies on a VALID or signed artifact as proof that the underlying evidence is sufficient.",
        publicDisclosure: "A valid signature and seal establish artifact integrity, not evidence sufficiency, operational safety, regulatory compliance, or permission to make external claims.",
        requiredEvidence: "The artifact must be VALID, signature verification must pass inside the declared trust boundary, accepted evidence coverage must be non-zero, integrity must be at least 0.60, the trust label must be HIGH_TRUST, and evidence readiness must be READY with signed evidence references before claims are eligible.",
        migration: "Reports generated under 2026.06.25-r220 or earlier must be regenerated or interpreted with derived evidence readiness; raw VALID or signed status alone no longer authorizes external claims."
      },
      {
      ...
      {
        boundary: "fact_checking_factuality_review_methodology_integrity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, benchmark receipt, or public claim uses the Artificial Intelligence Review fact-checking and factuality-evaluation review (DOI 10.1007/s10462-025-11454-w; OpenAlex W7118132038) as methodology support for hallucination detection, claim verification, factuality metrics, source grounding, evidence retrieval, evaluator calibration, or abstention/uncertainty controls.",
        publicDisclosure: "The DOI/OpenAlex title, abstract, venue, publication date, taxonomy labels, cited benchmark names, review conclusions, or source metadata alone do not establish AMC score comparability, a standalone fact-checking subsystem, or parity with any reviewed system. AMC uses the review only as methodology-versioning source-review evidence; no paper prose, figures, tables, benchmark data, prompts, taxonomies, or metric values are copied.",
        requiredEvidence: "Signed methodology id, methodology version, methodology hash, source-review id, DOI metadata receipt hash, OpenAlex metadata receipt hash, title/venue/year match, retrieval date, review-to-AMC control mapping hash, claim decomposition policy hash, source grounding policy hash, evidence retrieval manifest hash, verification-label taxonomy hash, factuality metric taxonomy hash, evaluator-calibration policy hash, uncertainty/abstention policy hash, source-freshness policy hash, benchmark-contamination policy hash, threshold policy hash, changelog row hash, deprecation notice hash, migration guidance hash, diagnostic methodology-versioning receipt hash, badge assurance hash, accepted evidence refs, rejected metadata-only evidence refs, signed evidence refs, and row hashes.",
        migration: "Reports generated under 2026.06.20-r213 should be regenerated or relabeled before using review-informed fact-checking, factuality evaluation, hallucination detection, claim verification, source grounding, evidence retrieval, abstention, or evaluator-calibration claims across Score, Shield, or Watch."
      },
      {
      ...
      {
        boundary: "arize_phoenix_eval_observability_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses Arize Phoenix/AX-style tracing, online evals on traces, offline evals on experiments, datasets, evaluator tasks, annotations, dashboards, production monitoring, alerting, or cost/latency evidence as proof of metric quality.",
        publicDisclosure: "A Phoenix or Arize AX label, phoenix.arize.com redirect, public docs page, docs index, product-page metadata, trace viewer, dashboard screenshot, evaluator name, dataset title, experiment row, aggregate eval score, monitor name, alert label, local export, or source metadata alone does not establish comparable AMC metric validity without AMC-owned eval pack, primary-source retrieval refs, trace/span export, evaluator task/config, dataset and experiment manifests, threshold policy, metric owner, sample size, confidence interval, signed evidence, and row hashes. This is not a parity claim and does not authorize copied Phoenix website prose, screenshots, prompts, examples, configs, or UI assets.",
        requiredEvidence: "Signed AMC eval-pack manifest, live primary-source docs retrieval refs with retrieval date, trace/span export manifest, evaluator task and evaluator config manifests, trace/session/span granularity declaration, dataset manifest, experiment/run manifest, annotation/eval-result export, production-monitoring or alert-policy manifest when Watch is claimed, Score/Shield/Watch surface mapping, fail-closed threshold policy, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes.",
        migration: "Reports generated under 2026.06.20-r211 should be regenerated or relabeled before using Arize Phoenix/AX-style tracing, online evals, offline evals, evaluator tasks, datasets, experiments, annotations, monitoring, alerts, cost/latency, thresholds, or metric-validity claims as external evidence."
      },
      {
        boundary: "mastra_eval_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses Mastra-style agent framework evaluations, scorers, traces, observability exports, workflow or tool-agent runs, evaluator dashboards, or threshold evidence as proof of metric quality.",
        publicDisclosure: "A Mastra label, GitHub repository metadata, topic list, package name, docs or README summary, evals directory, scorer filename, observability integration name, local framework run, dashboard screenshot, aggregate score, or source metadata alone does not establish comparable AMC metric validity without AMC-owned eval pack, validation table, evaluator-suite proof, trace-evaluation proof where tracing is claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence, no-copy boundary, and row hashes. This is not a Mastra subsystem or parity claim.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, signed AMC eval-pack manifest, validation table artifact, deterministic assertion or scorer manifest, LLM judge or custom judge config when used, dataset eval manifest, reporter output, framework integration proof, trace/measurement export manifest when tracing or Watch is claimed, fail-closed threshold policy, Score/Shield/Watch surface mapping, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.20-r212 should be regenerated or relabeled before using mastra-ai/mastra-style agent framework evals, scorers, traces, observability, workflow/tool-agent runs, thresholds, or metric-validity claims as external evidence."
      },
      {
        boundary: "lunary_observability_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses Lunary-style LLM observability, prompt templates, chat replay, analytics, topic classification, agent tracing, custom dashboards, response scoring, feedback tracking, PII masking, or monitoring alerts as proof of metric quality.",
        publicDisclosure: "A Lunary label, product-page metadata, website feature list, dashboard screenshot, prompt template name, trace id, analytics chart, response score, feedback widget, alert label, local export, or source metadata alone does not establish comparable AMC public methodology or metric validity without an AMC-owned eval pack, live source receipt, trace/session export, evaluator or response-scoring config, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed evidence, and row hashes. This is not a parity claim and does not authorize copied Lunary website prose, screenshots, prompts, examples, configs, or UI assets.",
        requiredEvidence: "Signed AMC eval-pack manifest, live source relevance receipt with retrieval date, trace/session export manifest, prompt-template or model-registry snapshot when claimed, evaluator or score-LLM-response config, feedback or annotation export when claimed, production-monitoring or alert-policy manifest when Watch is claimed, validation table artifact, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, Score/Shield/Watch surface mapping, fail-closed threshold policy, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes.",
        migration: "Reports generated under 2026.06.25-r218 should be regenerated or relabeled before using Lunary-style observability, prompt templates, agent tracing, response scoring, analytics, feedback tracking, monitoring alerts, thresholds, public-methodology, or metric-validity claims as external evidence."
      },
      {
        boundary: "cua_computer_use_public_methodology",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, methodology-versioning receipt, or metric-validation row uses trycua/cua-style computer-use agents, desktop sandboxes, Cua-Bench, OSWorld, ScreenSpot, Windows Arena, task datasets, desktop trajectories, or benchmark harness output as proof of public methodology quality, metric quality, or comparability.",
        publicDisclosure: "A Cua label, trycua/cua GitHub repository metadata, README summary, docs page, star/fork/issue count, topic list, default branch, license, sandbox label, OS label, Cua-Bench label, OSWorld/ScreenSpot/Windows Arena label, local benchmark command, aggregate score, trajectory file name, or source metadata alone does not establish comparable AMC methodology or metric validity without AMC-owned benchmark task manifests, environment image manifests, setup script and test script hashes, oracle solution proof, run config, trajectory export proof when claimed, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed evidence, no-copy proof, and row hashes. This is not a Cua integration and not a parity claim, and it does not authorize copied Cua README prose, docs, code, configs, examples, tasks, screenshots, trajectories, datasets, benchmark rows, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, repository snapshot hash, README snapshot hash, Cua-Bench docs snapshot hash, signed AMC eval-pack manifest, task dataset manifest, environment image manifest, setup script and test script hashes, oracle solution proof, evaluation or training harness config, run config, trajectory export manifest when claimed, validation table artifact, fail-closed threshold policy, Score/Shield/Watch surface mapping, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes.",
        migration: "Reports generated under 2026.06.25-r219 should be regenerated or relabeled before using trycua/cua-style, Cua-style, Cua-Bench, desktop sandbox, computer-use-agent, OSWorld, ScreenSpot, Windows Arena, task dataset, trajectory, threshold, public-methodology, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        boundary: "google_adk_eval_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses Google ADK-style agent toolkit evaluation, deployment, session, artifact, tool, or trace signals as proof of metric quality.",
        publicDisclosure: "A Google ADK label, google/adk-python GitHub repository metadata, default branch, license, star count, package description, README summary, docs page, module path, local framework run, dashboard screenshot, aggregate score, tool name, session label, or source metadata alone does not establish comparable AMC metric validity without an AMC-owned eval pack, validation table, evaluator-suite proof through existing AMC primitives, trace-evaluation proof where tracing or Watch is claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence, no-copy boundary, and row hashes. This is not a Google ADK subsystem, adapter, importer, or parity claim and does not authorize copied Google ADK code, prose, examples, prompts, configs, tests, docs, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, signed AMC eval-pack manifest, validation table artifact, evaluator-suite proof using existing primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, Score/Shield/Watch surface mapping, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.20-r213 should be regenerated or relabeled before using google/adk-python-style or Google ADK-style agent evaluation, deployment, sessions, artifacts, tools, traces, thresholds, or metric-validity claims as external evidence."
      },
      {
        boundary: "lm_evaluation_harness_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses LM Evaluation Harness-style evaluator, task, model benchmark, leaderboard, harness-run, or result signals as proof of metric quality.",
        publicDisclosure: "An LM Evaluation Harness label, EleutherAI/lm-evaluation-harness GitHub repository metadata, default branch, MIT license, star/fork/issue count, package description, README summary, docs page, task name, evaluator name, model name, local harness run, leaderboard row, aggregate score, or source metadata alone does not establish comparable AMC metric validity without an AMC-owned eval pack, validation table, evaluator-suite proof through existing AMC primitives, trace-evaluation proof where traces or Watch are claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence, no-copy boundary, and row hashes. This is not an lm-evaluation-harness subsystem, SDK, importer, adapter, or parity claim and does not authorize copied upstream code, prose, examples, prompts, configs, tests, docs, task definitions, result tables, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, signed AMC eval-pack manifest, validation table artifact, evaluator-suite proof using existing primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, Score/Shield/Watch surface mapping, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.20-r215 should be regenerated or relabeled before using EleutherAI/lm-evaluation-harness-style, LM Evaluation Harness, evaluator, task, model benchmark, leaderboard, harness-run, result, threshold, or metric-validity claims as external evidence."
      },
      {
        boundary: "openai_evals_public_methodology",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or methodology-versioning receipt uses OpenAI Evals-style evaluation framework, registry, benchmark, dataset, eval-run, or eval-result signals as proof of methodology quality, metric quality, or public comparability.",
        publicDisclosure: "An OpenAI Evals label, openai/evals GitHub repository metadata, default branch, license status, star/fork/issue count, README summary, docs path, registry path, package name, module path, local eval run, dataset name, eval name, aggregate score, or source metadata alone does not establish comparable AMC methodology or metric validity without AMC-owned eval packs, validation tables, evaluator-suite proof through existing AMC primitives, fail-closed threshold policy, methodology changelog, deprecation notice, migration guidance, signed evidence, no-copy boundary, and row hashes. This is not an OpenAI Evals subsystem, importer, adapter, parity claim, or registry mirror and does not authorize copied upstream code, prose, configs, prompts, datasets, eval specs, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, signed AMC eval-pack manifest, dataset/case manifest when claimed, validation table artifact, evaluator-suite proof using existing primitives, fail-closed threshold policy, Score/Shield/Watch surface mapping, methodology version/changelog/deprecation/migration proof, metric owner, sample size, and confidence interval where metric validity is claimed, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.20-r214 should be regenerated or relabeled before using openai/evals-style, OpenAI Evals, eval registry, benchmark, dataset, eval-run, evaluator, threshold, or public-methodology claims as external Score, Shield, or Watch evidence."
      },
      {
        boundary: "pocketflow_public_methodology",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or methodology-versioning receipt uses The-Pocket/PocketFlow-style lightweight agent workflow framework, docs, flow/node, agent, workflow, or minimal-framework signals as proof of methodology quality, metric quality, or public comparability.",
        publicDisclosure: "A PocketFlow label, The-Pocket/PocketFlow GitHub repository metadata, default branch, MIT license, star/fork/issue count, topic list, docs path, source module path, package/module name, local flow run, node or agent label, workflow example, aggregate score, or source metadata alone does not establish comparable AMC methodology or metric validity without AMC-owned eval packs, validation tables, evaluator-suite proof through existing AMC primitives, fail-closed threshold policy, methodology changelog, deprecation notice, migration guidance, signed evidence, no-copy boundary, and row hashes. This is not a PocketFlow subsystem, SDK, importer, adapter, parity claim, or framework compatibility layer and does not authorize copied upstream code, prose, configs, examples, prompts, docs, tests, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, docs/module scope declaration, signed AMC eval-pack manifest, flow/case manifest when claimed, validation table artifact, evaluator-suite proof using existing primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, Score/Shield/Watch surface mapping, methodology version/changelog/deprecation/migration proof, metric owner, sample size, and confidence interval where metric validity is claimed, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.21-r217 should be regenerated or relabeled before using The-Pocket/PocketFlow-style, PocketFlow, lightweight agent workflow framework, docs, flow/node, agent, workflow, minimal-framework, threshold, or public-methodology claims as external Score, Shield, or Watch evidence."
      },
      {
        boundary: "openai_simple_evals_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row uses OpenAI Simple Evals-style lightweight evaluator, task, benchmark, eval-run, result, or score signals as proof of metric quality.",
        publicDisclosure: "An OpenAI Simple Evals label, openai/simple-evals GitHub repository metadata, default branch, MIT license, star/fork/issue count, Python language metadata, README summary, module path, local eval run, task name, benchmark name, aggregate score, result row, or source metadata alone does not establish comparable AMC metric validity without an AMC-owned eval pack, validation table, evaluator-suite proof through existing AMC primitives, trace-evaluation proof where traces or Watch are claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence, no-copy boundary, and row hashes. This is not an OpenAI Simple Evals subsystem, SDK, importer, adapter, or parity claim and does not authorize copied upstream code, prose, examples, prompts, configs, tests, task definitions, result tables, or implementation details.",
        requiredEvidence: "Live GitHub metadata relevance review with retrieval date, signed AMC eval-pack manifest, validation table artifact, evaluator-suite proof using existing primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, Score/Shield/Watch surface mapping, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof.",
        migration: "Reports generated under 2026.06.20-r216 should be regenerated or relabeled before using openai/simple-evals-style, OpenAI Simple Evals, lightweight evaluator, task, benchmark, eval-run, result, threshold, or metric-validity claims as external Score, Shield, or Watch evidence."

      },
      {
        boundary: "digital_materials_ecosystem_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row cites Digital materials ecosystem, DOI 10.1039/d5sc09229a, OpenAlex W7131071926, or related autonomous-discovery/database/tool literature as proof of metric quality.",
        publicDisclosure: "A paper title, DOI record, OpenAlex record, journal metadata, abstract summary, citation label, database name, agent-workflow claim, autonomous-discovery label, or source metadata alone does not establish comparable AMC metric validity without an AMC-owned eval pack, validation table, threshold policy, metric owner, sample size, confidence interval, signed evidence, and row hashes. This is not a materials-domain subsystem and not a parity claim and does not authorize copied paper prose, figures, tables, datasets, prompts, workflows, or implementation details.",
        requiredEvidence: "Verified DOI metadata receipt, verified OpenAlex metadata receipt, metadata-only source-review proof with retrieval date, signed AMC eval-pack manifest, validation table artifact, existing metric-validity primitive mapping, trace-evaluation or evaluator-suite proof when claimed, Score/Shield/Watch surface mapping, fail-closed threshold policy, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes.",
        migration: "Reports generated under 2026.06.20-r213 should be regenerated or relabeled before using Digital materials ecosystem-cited autonomous-discovery, database/tool, research-workflow, or metric-validity claims as external evidence."
      },
      {
        boundary: "chemgraph_agentic_chemistry_workflow_metric_validity",
        appliesWhen: "Any Score report, Shield receipt, Watch alert, badge, API response, or metric-validation row cites ChemGraph, DOI 10.1038/s42004-025-01776-9, OpenAlex W7119161162, agentic computational chemistry workflows, task-planning, multi-agent reasoning, or simulation-workflow literature as proof of metric quality.",
        publicDisclosure: "A ChemGraph label, paper title, DOI record, OpenAlex record, journal metadata, abstract summary, computational-chemistry label, benchmark-task count, LLM/model name, multi-agent workflow claim, simulation-tool claim, aggregate accuracy, or source metadata alone does not establish comparable AMC metric validity without an AMC-owned eval pack, validation table, threshold policy, metric owner, sample size, confidence interval, signed evidence, and row hashes. This is not a chemistry/domain subsystem, connector, importer, or parity claim and does not authorize copied paper prose, figures, tables, benchmark rows, chemistry data, prompts, workflows, or implementation details.",
        requiredEvidence: "Verified DOI metadata receipt, verified OpenAlex metadata receipt, metadata-only source-review proof with retrieval date, signed AMC eval-pack manifest, validation table artifact, existing metric-validity primitive mapping, trace-evaluation or evaluator-suite proof when claimed, Score/Shield/Watch surface mapping, fail-closed threshold policy, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes.",
        migration: "Reports generated under 2026.06.20-r215 should be regenerated or relabeled before using ChemGraph-cited agentic computational chemistry workflow, task-planning, multi-agent, simulation-workflow, benchmark, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
    ...
    metricValidationGates: [
      ...
      {
        gate: "arize_phoenix_observability_eval_coverage",
        defaultThreshold: ">= 1.00 when required",
        appliesWhen: "Metrics that explicitly use Arize Phoenix/AX-style trace/span evaluation, datasets, experiments, evaluator tasks, annotations, production monitoring, or alerting as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].arizePhoenixObservabilityEvalCoverage",
        migration: "Set requireArizePhoenixObservabilityEvalProof and attach primary-source docs refs, trace/span export, evaluator task/config, dataset and experiment manifests, annotation/eval export, threshold policy, metric owner, sample-size, confidence-interval, signed-evidence refs, and row hashes before using Phoenix/AX-style observability-eval claims as external proof."
      },
      {
        gate: "lunary_observability_metric_validity",
        defaultThreshold: ">= 1.00 when required; validationTable present; methodologyVersion present; changelog present; deprecationNotice present; migrationGuidance present; badgeAssuranceHash present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only source review fails closed",
        appliesWhen: "Metrics that explicitly use Lunary-style LLM observability, prompt templates, chat replay, analytics, agent tracing, response scoring, feedback tracking, PII masking, or monitoring alerts as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].traceEvaluationCoverage and metricValidation.rows[].evaluatorSuiteCoverage via existing primitives",
        migration: "Attach a live source receipt, AMC-owned eval-pack manifest, trace/session export, prompt/model-registry snapshot when claimed, evaluator or response-scoring config, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, and row hashes. Product-page metadata alone fails closed."
      },
      {
        gate: "cua_computer_use_public_methodology",
        defaultThreshold: "complete when supplied; methodologyVersion present; changelog present; deprecationNotice present; migrationGuidance present; taskDatasetManifest present; environmentImageManifest present; testScriptOracleProof present; validationTable present; thresholdPolicy present; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Methodology-versioning or metric-validation rows explicitly use Cua-style computer-use agents, desktop sandboxes, Cua-Bench, OSWorld, ScreenSpot, Windows Arena, task datasets, trajectories, or benchmark harness evidence as Score, Shield, or Watch proof.",
        proofField: "methodologyVersioning.receiptHash and metricValidation.rows[].evaluatorSuiteCoverage plus metricValidation.rows[].traceEvaluationCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, README and Cua-Bench docs snapshots, AMC-owned eval-pack manifest, task dataset manifest, environment image manifest, setup script/test script/oracle proof, run config, trajectory export manifest when claimed, validation table, fail-closed threshold policy, methodology version/changelog/deprecation/migration proof, badge-assurance hash, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, row hashes, and no-copy/source-review proof. Repository metadata alone fails closed and no Cua adapter, SDK/importer, benchmark runner, sandbox integration, parity layer, or copied upstream content is introduced."
      },
      {
        gate: "google_adk_eval_metric_validity",
        defaultThreshold: ">= 1.00 when required; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Metrics that explicitly use Google ADK-style agent toolkit evaluation, deployment, sessions, artifacts, tools, traces, or threshold evidence as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].evaluatorSuiteCoverage and metricValidation.rows[].traceEvaluationCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, AMC-owned eval-pack manifest, validation table, fail-closed threshold policy, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, row hashes, and existing evaluator-suite and trace-evaluation primitives when traces or Watch are claimed. Repository metadata alone fails closed and no Google ADK subsystem, adapter, importer, parity layer, or copied upstream code/prose/config is introduced."
      },
      {
        gate: "lm_evaluation_harness_metric_validity",
        defaultThreshold: ">= 1.00 when required; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Metrics that explicitly use LM Evaluation Harness-style evaluators, tasks, model benchmarks, leaderboard rows, harness runs, results, or threshold evidence as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].evaluatorSuiteCoverage and metricValidation.rows[].traceEvaluationCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, AMC-owned eval-pack manifest, validation table, fail-closed threshold policy, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, row hashes, and existing evaluator-suite and trace-evaluation primitives when traces or Watch are claimed. Repository metadata alone fails closed and no lm-evaluation-harness subsystem, SDK/importer, adapter, parity layer, or copied upstream code/prose/config/task/result content is introduced."
      },
      {
        gate: "openai_evals_public_methodology",
        defaultThreshold: "complete when supplied; methodologyVersion present; changelog present; deprecationNotice present; migrationGuidance present; validationTable present when metric validity is claimed; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Methodology-versioning or metric-validation rows explicitly use OpenAI Evals-style evaluation-framework, registry, benchmark, dataset, eval-run, evaluator, threshold, or result evidence as Score, Shield, or Watch proof.",
        proofField: "methodologyVersioning.receiptHash and metricValidation.rows[].evaluatorSuiteCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, AMC-owned eval-pack manifest, dataset/case manifests when claimed, validation table, fail-closed threshold policy, methodology version/changelog/deprecation/migration proof, signed-evidence refs, artifact hashes, row hashes, and existing evaluator-suite primitives when metric validity is claimed. Repository metadata alone fails closed and no OpenAI Evals subsystem, importer, adapter, registry mirror, parity layer, or copied upstream code/prose/config/dataset/eval spec is introduced."
      },
      {
        gate: "pocketflow_public_methodology",
        defaultThreshold: "complete when supplied; methodologyVersion present; changelog present; deprecationNotice present; migrationGuidance present; docsModuleScope present; validationTable present when metric validity is claimed; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Methodology-versioning or metric-validation rows explicitly use PocketFlow-style lightweight agent workflow framework, docs, flow/node, agent, workflow, evaluator, threshold, or result evidence as Score, Shield, or Watch proof.",
        proofField: "methodologyVersioning.receiptHash and metricValidation.rows[].evaluatorSuiteCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, docs/module source-review scope, AMC-owned eval-pack manifest, flow/case manifests when claimed, validation table, fail-closed threshold policy, methodology version/changelog/deprecation/migration proof, signed-evidence refs, artifact hashes, row hashes, and existing evaluator-suite primitives when metric validity is claimed. Repository metadata alone fails closed and no PocketFlow subsystem, SDK/importer, adapter, parity layer, or copied upstream code/prose/config/examples is introduced."
      },
      {
        gate: "openai_simple_evals_metric_validity",
        defaultThreshold: ">= 1.00 when required; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only GitHub source review fails closed",
        appliesWhen: "Metrics that explicitly use OpenAI Simple Evals-style lightweight evaluators, tasks, benchmarks, local eval runs, results, or threshold evidence as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].evaluatorSuiteCoverage and metricValidation.rows[].traceEvaluationCoverage via existing primitives",
        migration: "Attach live GitHub metadata relevance review, AMC-owned eval-pack manifest, validation table, fail-closed threshold policy, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, row hashes, and existing evaluator-suite and trace-evaluation primitives when traces or Watch are claimed. Repository metadata alone fails closed and no OpenAI Simple Evals subsystem, SDK/importer, adapter, parity layer, or copied upstream code/prose/config/task/result content is introduced."

      },
      {
        gate: "digital_materials_ecosystem_metric_validity",
        defaultThreshold: ">= 1.00 when required; DOI/OpenAlex metadata verified; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only source review fails closed",
        appliesWhen: "Metrics that explicitly cite Digital materials ecosystem, DOI 10.1039/d5sc09229a, OpenAlex W7131071926, autonomous-discovery literature, database/tool literature, or research-workflow literature as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].scientificLiteratureCoverage, metricValidation.rows[].traceEvaluationCoverage, and metricValidation.rows[].evaluatorSuiteCoverage via existing primitives",
        migration: "Attach verified DOI/OpenAlex source-review receipts, AMC-owned eval-pack manifest, validation table, existing metric-validity primitive mapping, threshold policy, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, and row hashes. DOI/OpenAlex metadata alone fails closed."
      },
      {
        gate: "chemgraph_agentic_chemistry_workflow_metric_validity",
        defaultThreshold: ">= 1.00 when required; DOI/OpenAlex metadata verified; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only source review fails closed",
        appliesWhen: "Metrics that explicitly cite ChemGraph, DOI 10.1038/s42004-025-01776-9, OpenAlex W7119161162, agentic computational chemistry workflow literature, multi-agent task-planning literature, or simulation-workflow benchmark literature as Score, Shield, or Watch metric-validity proof.",
        proofField: "metricValidation.rows[].scientificLiteratureCoverage, metricValidation.rows[].traceEvaluationCoverage, and metricValidation.rows[].evaluatorSuiteCoverage via existing primitives",
        migration: "Attach verified DOI/OpenAlex source-review receipts, AMC-owned eval-pack manifest, validation table, existing metric-validity primitive mapping, threshold policy, metric owner, sample size, confidence interval, signed-evidence refs, artifact hashes, and row hashes. DOI/OpenAlex metadata alone fails closed and no chemistry/domain subsystem, connector, importer, parity layer, or copied paper prose/data is introduced."
      },
      {
      ...
      },
      {
        ...
        defaultThreshold: ">= 1.00 when required; methodologyVersion present; changelog present; deprecationNotice present; migrationGuidance present; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; failClosed on missing threshold policy; metadata-only source review fails closed",
      ...
      {
        gate: "fact_checking_factuality_review_methodology_evidence",
        defaultThreshold: "complete when supplied and DOI/OpenAlex metadata, mapping, threshold, signed-evidence, no-copy, changelog, deprecation, and migration proof are present",
        appliesWhen: "Score, Shield, or Watch rows that claim review-informed fact-checking, factuality-evaluation, hallucination-detection, claim-verification, source-grounding, evidence-retrieval, abstention, or evaluator-calibration methodology comparability from DOI 10.1007/s10462-025-11454-w / OpenAlex W7118132038.",
        proofField: "methodologyVersioning.receiptHash and metricValidation.rows[].factCheckingFactualityReviewMethodology",
        migration: "Attach DOI metadata receipt, OpenAlex metadata receipt, title/venue/year match, source-review id, review-to-AMC control mapping, claim decomposition policy, source grounding policy, evidence retrieval manifest, verification-label and factuality-metric taxonomies, evaluator-calibration policy, uncertainty/abstention policy, source-freshness policy, benchmark-contamination policy, threshold policy, changelog, deprecation notice, migration guidance, badge-assurance hash, signed evidence, no-copy proof, rejected metadata-only evidence, and row hashes before using fact-checking/factuality review methodology claims as external proof."
      },
      {
    ...
    limitations: [
      ...
      "Fact-checking and factuality-evaluation review methodology claims require DOI and OpenAlex metadata receipts for DOI 10.1007/s10462-025-11454-w / OpenAlex W7118132038, title/venue/year match, review-to-AMC control mapping, claim decomposition, source-grounding, evidence-retrieval, verification-label, factuality-metric, evaluator-calibration, uncertainty/abstention, source-freshness, contamination, threshold, changelog, deprecation, migration, badge-assurance, signed-evidence, no-copy, rejected metadata-only, and row-hash proof; paper metadata, abstract summaries, taxonomy labels, copied prose/figures/tables, benchmark names, or source metadata alone are not enough and do not create a standalone fact-checking subsystem.",
      ...
      "Arize Phoenix/AX-style observability and evaluation metric-validity claims require live primary-source docs retrieval refs, an AMC-owned eval pack, trace/span export manifest, evaluator task and config manifests, dataset and experiment manifests, annotation or eval-result export, production-monitoring or alert-policy manifest when Watch is claimed, fail-closed threshold policy, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes before Score, Shield, or Watch claims can use them; phoenix.arize.com redirects, public docs metadata, Phoenix or Arize AX labels, trace/dashboard screenshots, evaluator names, dataset titles, experiment rows, aggregate eval scores, alerts, local exports, copied website prose, copied examples, or source metadata alone are not enough and do not establish parity.",
      "Mastra-style agent framework eval-metric validity claims require live GitHub metadata relevance review, an AMC-owned eval pack, validation table artifact, deterministic assertion/scorer proof, LLM judge or custom judge config when used, dataset eval manifest, reporter output, framework integration proof, trace or measurement export evidence when tracing or Watch is claimed, fail-closed threshold policy, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof before Score, Shield, or Watch claims can use them; a mastra-ai/mastra repository label, topic list, evals package, scorer filename, docs/README summary, local framework run, dashboard screenshot, aggregate score, or source metadata alone is not enough and does not establish parity.",
      "Lunary-style public-methodology and metric-validity claims require a live source receipt, an AMC-owned eval pack, validation table, trace/session export manifest, prompt-template or model-registry snapshot when claimed, evaluator or response-scoring config, feedback or annotation export when claimed, production-monitoring or alert-policy manifest when Watch is claimed, fail-closed threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, no-copy/source-review boundary proof, signed evidence refs, artifact hashes, and row hashes before Score, Shield, or Watch claims can use them; product-page metadata, Lunary labels, dashboard screenshots, trace ids, response-score snippets, feedback widgets, alerts, copied website prose, copied examples, local exports, or source metadata alone are not enough and do not establish parity.",
      "Cua-style computer-use benchmark public-methodology claims require live GitHub relevance review, README and Cua-Bench docs snapshots, AMC-owned eval-pack and task dataset manifests, environment image manifests, setup script/test script/oracle proof, run config, trajectory export manifest when claimed, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before Score, Shield, or Watch claims can use them; a trycua/cua label, repository metadata, sandbox label, OS label, benchmark command, OSWorld/ScreenSpot/Windows Arena label, aggregate score, trajectory filename, copied Cua README prose, copied docs, copied tasks, copied trajectories, copied configs, or source metadata alone is not enough and does not establish a Cua subsystem, adapter, runner, sandbox integration, or parity claim.",
      "Google ADK-style agent toolkit eval-metric validity claims require live GitHub metadata relevance review, an AMC-owned eval pack, validation table, evaluator-suite proof through existing AMC primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof before Score, Shield, or Watch claims can use them; a google/adk-python repository label, default branch, license, star count, package description, README/docs summary, module path, tool/session label, local framework run, dashboard screenshot, aggregate score, copied upstream code/prose/config, or source metadata alone is not enough and does not establish parity or an AMC ADK subsystem/adapter/importer.",
      "LM Evaluation Harness-style eval-metric validity claims require live GitHub metadata relevance review, an AMC-owned eval pack, validation table, evaluator-suite proof through existing AMC primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review boundary proof before Score, Shield, or Watch claims can use them; an EleutherAI/lm-evaluation-harness repository label, default branch, MIT license, star/fork/issue count, README/docs summary, task/evaluator/model names, local harness run, leaderboard row, aggregate score, copied upstream code/prose/config/task/result content, or source metadata alone is not enough and does not establish parity or an AMC lm-evaluation-harness subsystem/SDK/importer.",
      "OpenAI Evals-style public-methodology claims require live GitHub metadata relevance review, AMC-owned eval-pack manifest, dataset/case manifests when claimed, validation table, evaluator-suite proof through existing primitives, fail-closed threshold policy, methodology id/version/hash, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner/sample size/confidence interval where metric-validity is claimed, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before Score, Shield, or Watch claims can use them; an openai/evals repository label, default branch, license, star/fork/issue count, README/docs/registry/module path, local eval run, aggregate score, copied upstream code/prose/config/dataset/eval spec, or source metadata alone is not enough and does not establish parity or an AMC OpenAI Evals subsystem/importer.",
      "ChemGraph-style agentic computational chemistry workflow metric-validity claims require DOI/OpenAlex metadata verification, AMC-owned eval-pack manifest, validation table, existing metric-validity primitive mapping, trace/evaluator proof when claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before Score, Shield, or Watch claims can use them; DOI metadata, OpenAlex metadata, paper title, abstract summary, computational-chemistry label, benchmark-task count, LLM/model name, aggregate accuracy, workflow claim, or source metadata alone is not enough and does not establish a chemistry/domain subsystem, connector, importer, or parity claim.",
    ...
    changelog: [
      ...
      {
        version: "2026.06.25-r220",
        date: "2026-06-25",
        summary: "Adds Cua-style computer-use benchmark public-methodology assurance binding for Score, Shield, Watch, diagnostics, and badges so trycua/cua repository, README, docs, sandbox, Cua-Bench, OSWorld, ScreenSpot, Windows Arena, task, trajectory, and benchmark-run metadata remain source-review signals only while AMC-owned eval-pack/task/environment/test-oracle proof, validation tables, threshold policies, methodology version/changelog/deprecation/migration proof, badge assurance, metric owners, sample sizes, confidence intervals, signed evidence, no-copy proof, and row hashes fail closed without adding Cua adapters, SDK/importers, benchmark runners, sandbox integrations, parity layers, or copied upstream content.",
        migration: "Reports generated under 2026.06.25-r219 should be regenerated before using Cua-style computer-use agents, desktop sandboxes, Cua-Bench, OSWorld, ScreenSpot, Windows Arena, task datasets, trajectories, public-methodology, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.25-r219",
        date: "2026-06-25",
        summary: "Adds Lunary-style public-methodology source-review assurance binding for Score, Shield, Watch, diagnostics, and badges so product-page, docs, observability, prompt, tracing, response-scoring, feedback, and alert metadata remain source-review signals only while AMC-owned eval-pack proof, validation tables, threshold policies, methodology version/changelog/deprecation/migration proof, badge assurance, metric owners, sample sizes, confidence intervals, signed evidence, no-copy proof, and row hashes fail closed without adding Lunary adapters, importers, SDK integrations, dashboard clones, parity layers, or copied website prose.",
        migration: "Reports generated under 2026.06.25-r218 should be regenerated before using Lunary-style observability, prompt templates, agent tracing, response scoring, feedback tracking, monitoring alerts, public-methodology, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.25-r218",
        date: "2026-06-25",
        summary: "Adds LangSmith-style public-methodology source-review assurance binding for Score, Shield, Watch, diagnostics, and badges so live product-page, observability, and evaluation metadata remain source-review signals only while AMC-owned eval-pack proof, validation tables, threshold policies, methodology version/changelog/deprecation/migration proof, metric owners, sample sizes, confidence intervals, signed evidence, no-copy proof, badge assurance, and row hashes fail closed without adding LangSmith adapters, importers, SDK integrations, parity layers, or copied website prose.",
        migration: "Reports generated under 2026.06.21-r217 should be regenerated before using LangSmith-style observability, tracing, datasets, experiments, evaluator feedback, monitoring, cost/latency, threshold, public-methodology, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.21-r217",
        date: "2026-06-21",
        summary: "Adds PocketFlow-style public-methodology source-review boundaries and OpenAI Simple Evals-style metric-validity source-review boundaries for Score, Shield, Watch, diagnostics, and badges so live GitHub metadata, docs/module paths, and lightweight evaluator/task labels remain source-review signals only while AMC-owned eval-pack/validation proof, existing evaluator-suite or trace-evaluation primitives, threshold policies, methodology version/changelog/deprecation/migration proof, metric owners, sample sizes, confidence intervals where applicable, signed evidence, no-copy/source-review proof, badge assurance, and row hashes fail closed without adding PocketFlow or OpenAI Simple Evals subsystems, SDK/importers, adapters, parity layers, registry mirrors, framework-compatibility claims, or copied upstream code/prose/config/examples/task/result content.",
        migration: "Reports generated under 2026.06.20-r216 should be regenerated before using The-Pocket/PocketFlow-style, PocketFlow, lightweight agent workflow framework, docs, flow/node, agent, workflow, minimal-framework, openai/simple-evals-style, OpenAI Simple Evals, lightweight evaluator, task, benchmark, eval-run, result, threshold, public-methodology, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.20-r216",
        date: "2026-06-20",

        summary: "Adds ChemGraph-style agentic computational chemistry workflow metric-validity boundaries and LM Evaluation Harness-style metric-validity source-review boundaries for Score, Shield, Watch, diagnostics, and badges so DOI/OpenAlex or live GitHub metadata remains a source-review signal only while AMC-owned validation tables, existing metric-validity primitive mappings, evaluator-suite or trace-evaluation proof where claimed, threshold policies, metric owners, sample sizes, confidence intervals, signed evidence, no-copy/source-review proof, badge assurance, and row hashes fail closed without adding chemistry/domain or lm-evaluation-harness subsystems, connectors, SDK/importers, adapters, parity layers, or copied paper/upstream code/prose/config/task/result content.",
        migration: "Reports generated under 2026.06.20-r215 should be regenerated before using ChemGraph-cited agentic computational chemistry workflow, task-planning, multi-agent, simulation-workflow, benchmark, EleutherAI/lm-evaluation-harness-style, LM Evaluation Harness, evaluator, task, model benchmark, leaderboard, harness-run, result, threshold, or metric-validity claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.20-r215",
        date: "2026-06-20",
        summary: "Adds OpenAI Evals-style public-methodology source-review boundaries for Score, Shield, Watch, diagnostics, and badges so live GitHub metadata remains a source signal only while methodology version, changelog, deprecation notice, migration guidance, AMC-owned eval-pack/validation proof, fail-closed thresholds, signed evidence, no-copy/source-review proof, badge assurance, and row hashes fail closed with no OpenAI Evals subsystem, importer, adapter, registry mirror, parity layer, or copied upstream code/prose/config/dataset/eval spec.",
        migration: "Reports generated under 2026.06.20-r214 should be regenerated before using openai/evals-style, OpenAI Evals, eval registry, benchmark, dataset, eval-run, evaluator, threshold, or public-methodology claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.20-r214",
        date: AMC_PUBLIC_METHODOLOGY_RELEASE_DATE,
        summary: "Adds DOI/OpenAlex-verified fact-checking and factuality-evaluation review methodology boundaries plus Google ADK-style agent-toolkit evaluation metric-validity boundaries for Score, Shield, and Watch so source metadata remains a source signal only while AMC-owned validation/methodology proof, thresholds, signed evidence, no-copy/source-review proof, badge assurance, and row hashes fail closed with no standalone fact-checking subsystem and no ADK subsystems, adapters, importers, parity layers, or copied upstream code/prose/config.",
        migration: "Reports generated under 2026.06.20-r213 should be regenerated before using review-informed fact-checking/factuality or google/adk-python-style agent evaluation claims as external Score, Shield, or Watch evidence."
      },
      {
        version: "2026.06.20-r213",
        date: AMC_PUBLIC_METHODOLOGY_RELEASE_DATE,
        summary: "Adds Mastra-style agent-framework and Lunary-style observability/evaluation metric-validity boundaries for Score, Shield, and Watch so source metadata remains a source signal only while validation tables, evaluator proof, trace/session exports where claimed, threshold policies, metric owners, sample sizes, confidence intervals, signed evidence, no-copy/source-review proof, and row hashes fail closed.",
        migration: "Reports generated under 2026.06.20-r212 should be regenerated before using Mastra-style agent-framework evals or Lunary-style observability/evaluation claims as external evidence."
      },
      {
        version: "2026.06.20-r212",
        date: AMC_PUBLIC_METHODOLOGY_RELEASE_DATE,
        summary: "Adds Arize Phoenix/AX-style observability/evaluation metric-validity boundaries for Score, Shield, and Watch so primary-source docs retrieval, trace/span exports, evaluator task/config manifests, dataset and experiment manifests, annotation/eval exports, monitoring/alert policies, metric owners, sample sizes, confidence intervals, fail-closed thresholds, signed evidence, no-copy/source-review proof, and row hashes fail closed; docs and product-page metadata remain source-signal only.",
        migration: "Reports generated under 2026.06.20-r211 should be regenerated before using Arize Phoenix/AX-style tracing, online evals, offline evals, evaluator tasks, datasets, experiments, annotations, monitoring, alerts, cost/latency, thresholds, or metric-validity claims as external evidence."
      },
      {
        version: "2026.06.20-r211",
        date: AMC_PUBLIC_METHODOLOGY_RELEASE_DATE,
    ...
    migrationGuidance: [
      ...
      "For LangSmith-style public-methodology, observability, and evaluation claims, attach live source retrieval for the LangSmith product, observability, and evaluation pages, AMC-owned eval-pack manifests, dataset/experiment/trace manifests, evaluator or feedback config, validation table, threshold policy, methodology version/changelog/deprecation/migration proof, metric owner, sample size, confidence interval, signed evidence refs, row hashes, and no-copy/source-review proof before using Score, Shield, or Watch outputs externally; do not add a LangSmith adapter, SDK integration, importer, parity layer, or copied website prose.",
      "For Arize Phoenix/AX-style observability/evaluation claims, attach live primary-source docs retrieval refs, AMC-owned eval-pack manifests, trace/span exports, evaluator task/config manifests, dataset and experiment manifests, annotation/eval exports, threshold policies, signed evidence refs, row hashes, and no-copy proof before using Score, Shield, or Watch outputs externally.",
      "For Lunary-style public-methodology and observability/evaluation claims, attach the live relevance receipt, AMC-owned eval-pack manifest, trace/session export, prompt/model-registry snapshot when claimed, evaluator or response-scoring config, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed evidence refs, row hashes, and no-copy proof before using Score, Shield, or Watch outputs externally; do not add a Lunary adapter, SDK integration, importer, dashboard clone, parity layer, or copied website prose.",
      "For Cua-style computer-use benchmark public-methodology claims, attach live GitHub relevance review for trycua/cua, repository/README/Cua-Bench docs snapshots, AMC-owned eval-pack and task dataset manifests, environment image manifest, setup script/test script/oracle proof, run config, trajectory export manifest when claimed, validation table, threshold policy, methodology version, changelog, deprecation notice, migration guidance, badge-assurance hash, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy proof before using Score, Shield, or Watch outputs externally; do not add a Cua adapter, SDK/importer, benchmark runner, sandbox integration, parity layer, or copied upstream code/prose/config/tasks/trajectories.",
      "For fact-checking/factuality-review-informed claims, attach DOI/OpenAlex metadata receipts for 10.1007/s10462-025-11454-w / W7118132038, title/venue/year match, source-review id, control mapping, claim decomposition, source-grounding and evidence-retrieval manifests, factuality metric/evaluator-calibration/abstention policies, threshold policy, signed evidence refs, row hashes, and no-copy proof before using Score, Shield, or Watch outputs externally.",
      "For Google ADK-style agent evaluation claims, attach live GitHub metadata relevance review, AMC-owned eval-pack manifest, validation table, evaluator-suite proof, trace-evaluation proof when traces or Watch are claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence refs, row hashes, and no-copy/source-review proof before using Score, Shield, or Watch outputs externally; do not add an ADK subsystem, adapter, importer, parity layer, or copied upstream code/prose/config.",
      "For LM Evaluation Harness-style metric-validity claims, attach live GitHub metadata relevance review for EleutherAI/lm-evaluation-harness, AMC-owned eval-pack manifest, validation table, evaluator-suite proof through existing primitives, trace-evaluation proof when traces or Watch are claimed, threshold policy, metric owner, sample size, confidence interval, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before using Score, Shield, or Watch outputs externally; do not add an lm-evaluation-harness subsystem, SDK/importer, adapter, parity layer, or copied upstream code/prose/config/task/result content.",
      "For OpenAI Evals-style public-methodology claims, attach live GitHub metadata relevance review for openai/evals, AMC-owned eval-pack manifest, dataset/case manifests when claimed, validation table, evaluator-suite proof through existing primitives, fail-closed threshold policy, methodology version/changelog/deprecation/migration proof, badge-assurance hash, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before using Score, Shield, or Watch outputs externally; do not add an OpenAI Evals subsystem, importer, adapter, registry mirror, parity layer, or copied upstream code/prose/config/dataset/eval spec.",
      "For PocketFlow-style public-methodology claims, attach live GitHub metadata relevance review for The-Pocket/PocketFlow, docs/module source-review scope, AMC-owned eval-pack manifest, flow/case manifests when claimed, validation table, evaluator-suite proof through existing primitives, trace-evaluation proof when traces or Watch are claimed, fail-closed threshold policy, methodology version/changelog/deprecation/migration proof, badge-assurance hash, signed evidence refs, artifact hashes, row hashes, and no-copy/source-review proof before using Score, Shield, or Watch outputs externally; do not add a PocketFlow subsystem, SDK/importer, adapter, parity layer, or copied upstream code/prose/config/examples.",
  ...
  for (const entry of manifestWithoutHash.changelog.slice(1)) {
    const versionDate = /^(\d{4})\.(\d{2})\.(\d{2})/.exec(entry.version);
    if (versionDate) {
      entry.date = `${versionDate[1]}-${versionDate[2]}-${versionDate[3]}`;
    }
  }
