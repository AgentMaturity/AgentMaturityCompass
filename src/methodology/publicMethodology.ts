  ...
  const manifestWithoutHash = {
    ...
    metricValidationGates: [
      ...
      },
      {
        ...
        defaultThreshold: ">= 1.00 when required",
        appliesWhen: "Metrics that claim pentesting-agent or threat-model benchmark validity over vulnerable apps, multi-step exploit chains, flags, ground truth, false-positive traps, security controls, execution traces, or structured threat-model reports.",
        proofField: "metricValidation.rows[].pentestBenchmarkCoverage",
        migration: "Set requirePentestBenchmarkProof and attach Dockerized app manifest, language-stack coverage, vulnerability-class coverage, difficulty distribution, multi-step chain, flag ground truth, threat-model ground truth, false-positive trap, security-control, exploit trace, threat-model report, metric owner, sample-size, and confidence-interval evidence refs before using pentest or threat-model metric-validity claims as external proof."
      },
      {
    ...
    changelog: [
      {
        ...
        summary: "Adds Open Models RAG question-explainability receipts so source/repository/license-or-no-license/default-branch, README, Java source, build/dependency, LangChain4j, Ollama runtime, RAG pipeline, corpus, embedding, retrieval, evaluation, open model, question trace, evaluator config, metric result, score breakdown, rejected-evidence reasons, repair hints, regression thresholds, CI receipt, no-source-copy proof, signed evidence, and row hashes fail closed.",
        migration: "Reports generated under 2026.06.19-r209 should be regenerated before using bbenz/gen-ai-with-open-models-style, Open Models, Java local inference, LangChain4j, Ollama, RAG pipeline, RAG evaluation, or question score explainability claims as external evidence."
      },
      {
