...
export const AMC_PUBLIC_METHODOLOGY_VERSION = "2026.06.20-r213";
...
export function getPublicMethodologyManifest(questionSet?: DiagnosticQuestionSetInfo): PublicMethodologyManifest {
  const manifestWithoutHash = {
    ...
    deprecationNotice: "Reports and badges that do not include methodology.id, methodology.version, methodology.hash, and questionSet.version are legacy outputs and should not be used as current external proof. Reports generated before 2026.06.20-r213 must not present Lunary-style observability, prompt-template, agent-tracing, response-scoring, feedback-tracking, monitoring-alert, or metric-validity claims as externally comparable proof without a fresh methodology block, live source receipt, signed evidence refs, fail-closed threshold policy, validation table, metric owner, sample size, confidence interval, and no-copy/source-review receipt.",
    migrationGuidance: [
