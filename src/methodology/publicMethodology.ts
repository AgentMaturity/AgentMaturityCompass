...
export const AMC_PUBLIC_METHODOLOGY_VERSION = "2026.06.25-r218";
  ...
  const manifestWithoutHash = {
    ...
    benchmarkMethodologyVersioning: {
      ...
      changeTriggers: [
        ...
        },
        {
          ...
          versionImpact: "Increment corpus, verification-protocol, scoring, and methodology versions when Lunary-style observability/evaluation semantics change live source receipt, trace/session export, prompt or model registry, score/evaluator config, feedback export, monitoring/alert policy, validation table, threshold, metric owner, sample-size, confidence-interval, signed-evidence, row-hash, or no-copy/source-review requirements.",
          migration: "Regenerate badges or reports before comparing Lunary-style LLM observability, prompt templates, agent tracing, response scoring, analytics, feedback tracking, monitoring alerts, or metric-validity claims. Product-page metadata can seed review only; it is not parity proof."
        },
        {
    ...
    metricValidationGates: [
      ...
      },
      {
        ...
        defaultThreshold: ">= 1.00 when required; validationTable present; sampleSize>=configured minimum; confidenceInterval present; metricOwner present; signedEvidenceRefs present; metadata-only source review fails closed",
    ...
    migrationGuidance: [
      ...
      "For Lunary-style observability/evaluation claims, attach the live relevance receipt, AMC-owned eval-pack manifest, trace/session export, prompt/model-registry snapshot when claimed, evaluator or response-scoring config, validation table, threshold policy, metric owner, sample size, confidence interval, signed evidence refs, row hashes, and no-copy proof before using Score, Shield, or Watch outputs externally.",
