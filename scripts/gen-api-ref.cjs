...
function extractCliCommandsFromInventory() {
  const inventoryPath = path.join(ROOT, 'docs', 'CLI_COMMAND_INVENTORY.md');
  if (!fs.existsSync(inventoryPath)) {
    return null;
  }

  const body = fs.readFileSync(inventoryPath, 'utf8');
  const commands = [];
  for (const line of body.split('\n')) {
    ...
    const command = cells[0]?.match(/^`amc ([^`]+)`$/)?.[1];
    if (!command) {
      continue;
    }

    const description = (cells[1] || '').replaceAll('\\|', '|');
    const optionCell = cells[2] || '-';
    const options = optionCell === '-'
      ? []
      : optionCell.split('<br>').map((flag) => ({
    ...
    const aliases = aliasCell === '-'
      ? []
      : aliasCell.split(', ').map((alias) => alias.replaceAll('`', ''));
    commands.push({ name: command, description, options, aliases, line: commands.length + 1 });
  }
...
function extractCliCommands() {
  const inventoryCommands = extractCliCommandsFromInventory();
...
const commandCount = commands.length.toLocaleString('en-US');
...
AMC provides ${commandCount} public CLI command paths in the live command inventory.

| # | Command | Description |
|---|---------|-------------|
${commands.map((c, i) => `| ${i+1} | \`amc ${c.name}\` | ${c.description} |`).join('\n')}

### Command Details

${commands.filter(c => c.options.length > 0 || c.aliases?.length > 0).map(c => `#### \`amc ${c.name}\`

${c.description}

${c.aliases?.length > 0 ? `Alias: ${c.aliases.map(alias => `\`amc ${alias}\``).join(', ')}
` : ''}
| Option | Description |
|--------|-------------|
${c.options.length > 0 ? c.options.map(o => `| \`${o.flag}\` | ${o.desc || '-'} |`).join('\n') : '| - | - |'}
`).join('\n')}
